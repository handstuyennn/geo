...
public:
	ChainBuilder(const CoordinateSequence *pts, void *context, std::vector<MonotoneChain> &list)
	    : m_prev(nullptr), m_i(0), m_quadrant(-1), m_start(0), m_seq(pts), m_context(context), m_list(list) {
	}
...
/* static public */
void MonotoneChainBuilder::getChains(const CoordinateSequence *pts, void *context, std::vector<MonotoneChain> &mcList) {
