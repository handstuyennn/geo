...
/*public static*/
std::unique_ptr<std::vector<Coordinate>> KdTree::toCoordinates(std::vector<KdNode *> &kdnodes) {
	return toCoordinates(kdnodes, false);
}

/*public static*/
std::unique_ptr<std::vector<Coordinate>> KdTree::toCoordinates(std::vector<KdNode *> &kdnodes, bool includeRepeated) {
...
/*private*/
KdNode *KdTree::createNode(const Coordinate &p, void *data) {
	auto it = nodeQue.emplace(nodeQue.end(), p, data);
	return &(*it);
}

/*public*/
KdNode *KdTree::insert(const Coordinate &p) {
	return insert(p, nullptr);
}

/*public*/
KdNode *KdTree::insert(const Coordinate &p, void *data) {
	if (root == nullptr) {
...
/*private*/
KdNode *KdTree::findBestMatchNode(const Coordinate &p) {
	BestMatchVisitor visitor(p, tolerance);
	query(visitor.queryEnvelope(), visitor);
	return visitor.getNode();
}

KdNode *KdTree::insertExact(const geom::Coordinate &p, void *data) {
...
/*private*/
void KdTree::queryNode(KdNode *currentNode, const geom::Envelope &queryEnv, bool odd, KdNodeVisitor &visitor) {
...
/*private*/
KdNode *KdTree::queryNodePoint(KdNode *currentNode, const geom::Coordinate &queryPt, bool odd) {
	while (currentNode != nullptr) {
...
/*public*/
void KdTree::query(const geom::Envelope &queryEnv, KdNodeVisitor &visitor) {
	queryNode(root, queryEnv, true, visitor);
}

/*public*/
std::unique_ptr<std::vector<KdNode *>> KdTree::query(const geom::Envelope &queryEnv) {
...
/*public*/
void KdTree::query(const geom::Envelope &queryEnv, std::vector<KdNode *> &result) {
	AccumulatingVisitor visitor(result);
	queryNode(root, queryEnv, true, visitor);
}

/*public*/
KdNode *KdTree::query(const geom::Coordinate &queryPt) {
...
KdTree::BestMatchVisitor::BestMatchVisitor(const geom::Coordinate &p_p, double p_tolerance)
    : tolerance(p_tolerance), matchNode(nullptr), matchDist(0.0), p(p_p) {
}

/*private*/
Envelope KdTree::BestMatchVisitor::queryEnvelope() {
...
/*private*/
KdNode *KdTree::BestMatchVisitor::getNode() {
	return matchNode;
}

/*private*/
void KdTree::BestMatchVisitor::visit(KdNode *node) {
