...
public:
	PrecisionReducerFilter(bool p_removeRepeated, const PrecisionModel &p_filterPM)
	    : removeRepeated(p_removeRepeated), filterPM(p_filterPM) {
		m_prev.setNull();
	}
...
/* public static */
std::unique_ptr<Geometry> PrecisionReducerTransformer::reduce(const Geometry &geom, const PrecisionModel &targetPM,
...
/* private */
void PrecisionReducerTransformer::extend(std::vector<Coordinate> &coords, std::size_t minLength) {
...
std::unique_ptr<CoordinateSequence> PrecisionReducerTransformer::transformCoordinates(const CoordinateSequence *coords,
...
// protected
std::unique_ptr<Geometry> PrecisionReducerTransformer::transformPolygon(const Polygon *geom, const Geometry *parent) {
	(void)(parent); // ignore unused variable
	return reduceArea(geom);
}

// protected
std::unique_ptr<Geometry> PrecisionReducerTransformer::transformMultiPolygon(const MultiPolygon *geom,
                                                                             const Geometry *parent) {
	(void)(parent); // ignore unused variable
	return reduceArea(geom);
}
...
// private
std::unique_ptr<Geometry> PrecisionReducerTransformer::reduceArea(const Geometry *geom) {
