...
#include <geos/edgegraph/HalfEdge.hpp>
...
/*public static*/
HalfEdge *HalfEdge::create(const Coordinate &p0, const Coordinate &p1) {
...
/*public*/
void HalfEdge::link(HalfEdge *p_sym) {
	setSym(p_sym);
	p_sym->setSym(this);
...
/*public*/
HalfEdge *HalfEdge::find(const Coordinate &p_dest) {
	HalfEdge *oNxt = this;
	do {
		if (oNxt == nullptr) {
			return nullptr;
...
/*public*/
bool HalfEdge::equals(const Coordinate &p0, const Coordinate &p1) const {
	return m_orig.equals2D(p0) && m_sym->m_orig.equals2D(p1);
}

/*public*/
void HalfEdge::insert(HalfEdge *eAdd) {
...
/*private*/
HalfEdge *HalfEdge::insertionEdge(HalfEdge *eAdd) {
	HalfEdge *ePrev = this;
	do {
...
/*private*/
void HalfEdge::insertAfter(HalfEdge *e) {
...
/*public*/
bool HalfEdge::isEdgesSorted() const {
...
/*private*/
const HalfEdge *HalfEdge::findLowest() const {
...
/*public*/
int HalfEdge::compareAngularDirection(const HalfEdge *e) const {
...
std::ostream &operator<<(std::ostream &os, const HalfEdge &e) {
	os << "HE(" << e.m_orig.x << " " << e.m_orig.y << ", " << e.m_sym->m_orig.x << " " << e.m_sym->m_orig.y << ")";
	return os;
}

/*public static*/
void HalfEdge::toStringNode(const HalfEdge *he, std::ostream &os) {
