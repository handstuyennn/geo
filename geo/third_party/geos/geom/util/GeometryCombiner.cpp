...
std::unique_ptr<Geometry> GeometryCombiner::combine(std::vector<std::unique_ptr<Geometry>> &&geoms) {
	GeometryCombiner combiner(std::move(geoms));
	return combiner.combine();
}

std::unique_ptr<Geometry> GeometryCombiner::combine(std::vector<const Geometry *> const &geoms) {
	GeometryCombiner combiner(geoms);
	return combiner.combine();
}

std::unique_ptr<Geometry> GeometryCombiner::combine(const Geometry *g0, const Geometry *g1) {
...
std::unique_ptr<Geometry> GeometryCombiner::combine(std::unique_ptr<Geometry> &&g0, std::unique_ptr<Geometry> &&g1) {
...
std::unique_ptr<Geometry> GeometryCombiner::combine(const Geometry *g0, const Geometry *g1, const Geometry *g2) {
	std::vector<const Geometry *> geoms;
	geoms.push_back(g0);
	geoms.push_back(g1);
	geoms.push_back(g2);
...
std::unique_ptr<Geometry> GeometryCombiner::combine() {
	auto geomFactory = inputGeoms.empty() ? GeometryFactory::getDefaultInstance() : inputGeoms.front()->getFactory();

	if (skipEmpty) {
		inputGeoms.erase(std::remove_if(inputGeoms.begin(), inputGeoms.end(),
		                                [](std::unique_ptr<Geometry> &g) { return g->isEmpty(); }),
		                 inputGeoms.end());
	}
