...
#include <geos/geom/util/GeometryEditorOperation.hpp>
#include <typeinfo>
#include <vector>
 ...
 */
std::unique_ptr<Geometry> GeometryEditor::edit(const Geometry *geometry, GeometryEditorOperation *operation) {
	// if client did not supply a GeometryFactory, use the one from the input Geometry
	if (factory == nullptr) {
		factory = geometry->getFactory();
	}

	if (const GeometryCollection *gc = dynamic_cast<const GeometryCollection *>(geometry)) {
...
std::unique_ptr<Polygon> GeometryEditor::editPolygon(const Polygon *polygon, GeometryEditorOperation *operation) {
	std::unique_ptr<Polygon> newPolygon(detail::down_cast<Polygon *>(operation->edit(polygon, factory).release()));
	...
	std::unique_ptr<LinearRing> shell(
	    detail::down_cast<LinearRing *>(edit(newPolygon->getExteriorRing(), operation).release()));
	...
	for (std::size_t i = 0, n = newPolygon->getNumInteriorRing(); i < n; ++i) {
		...
		std::unique_ptr<LinearRing> hole(
		    detail::down_cast<LinearRing *>(edit(newPolygon->getInteriorRingN(i), operation).release()));
...
std::unique_ptr<GeometryCollection> GeometryEditor::editGeometryCollection(const GeometryCollection *collection,
	...
	for (std::size_t i = 0, n = newCollection->getNumGeometries(); i < n; i++) {
		auto geometry = edit(newCollection->getGeometryN(i), operation);
		if (geometry->isEmpty()) {
			continue;
		}
