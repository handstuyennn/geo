...
#include <cassert>
#include <cmath>
#include <geos/geom/DefaultCoordinateSequenceFactory.hpp>
...
public:
	gfCoordinateOperation(const CoordinateSequenceFactory *gsf) : _gsf(gsf) {
	}
	std::unique_ptr<CoordinateSequence> edit(const CoordinateSequence *coordSeq, const Geometry *) override {
...
/*public static*/
GeometryFactory::Ptr GeometryFactory::create() {
	return GeometryFactory::Ptr(new GeometryFactory());
}

/*protected*/
GeometryFactory::GeometryFactory()
    : SRID(0), coordinateListFactory(DefaultCoordinateSequenceFactory::instance()), _refCount(0), _autoDestroy(false) {
}

/*protected*/
GeometryFactory::GeometryFactory(const PrecisionModel *pm, int newSRID,
                                 CoordinateSequenceFactory *nCoordinateSequenceFactory)
    : SRID(newSRID), _refCount(0), _autoDestroy(false) {
	if (pm) {
		precisionModel = *pm;
	}
...
GeometryFactory::GeometryFactory(CoordinateSequenceFactory *nCoordinateSequenceFactory)
    : SRID(0), _refCount(0), _autoDestroy(false) {
	if (!nCoordinateSequenceFactory) {
		coordinateListFactory = DefaultCoordinateSequenceFactory::instance();
	} else {
		coordinateListFactory = nCoordinateSequenceFactory;
...
/*protected*/
GeometryFactory::GeometryFactory(const PrecisionModel *pm, int newSRID)
    : SRID(newSRID), coordinateListFactory(DefaultCoordinateSequenceFactory::instance()), _refCount(0),
      _autoDestroy(false) {
	if (pm) {
		precisionModel = *pm;
	}
...
GeometryFactory::GeometryFactory(const GeometryFactory &gf)
    : precisionModel(gf.precisionModel), SRID(gf.SRID), coordinateListFactory(gf.coordinateListFactory), _refCount(0),
      _autoDestroy(false) {
}

/*public static*/
GeometryFactory::Ptr GeometryFactory::create(const PrecisionModel *pm, int newSRID,
                                             CoordinateSequenceFactory *nCoordinateSequenceFactory) {
	return GeometryFactory::Ptr(new GeometryFactory(pm, newSRID, nCoordinateSequenceFactory));
}

/*public static*/
GeometryFactory::Ptr GeometryFactory::create(CoordinateSequenceFactory *nCoordinateSequenceFactory) {
	return GeometryFactory::Ptr(new GeometryFactory(nCoordinateSequenceFactory));
}

/*public static*/
GeometryFactory::Ptr GeometryFactory::create(const PrecisionModel *pm) {
	return GeometryFactory::Ptr(new GeometryFactory(pm));
}

/*public static*/
GeometryFactory::Ptr GeometryFactory::create(const PrecisionModel *pm, int newSRID) {
	return GeometryFactory::Ptr(new GeometryFactory(pm, newSRID));
}

/*public static*/
GeometryFactory::Ptr GeometryFactory::create(const GeometryFactory &gf) {
	return GeometryFactory::Ptr(new GeometryFactory(gf));
}
...
/*public*/
Geometry *GeometryFactory::buildGeometry(std::vector<Geometry *> *newGeoms) const {
	if (newGeoms->empty()) {
...
std::unique_ptr<Geometry> GeometryFactory::buildGeometry(std::vector<std::unique_ptr<Geometry>> &&geoms) const {
	if (geoms.empty()) {
	...
	default:
		return createGeometryCollection(std::move(geoms));
	}
}

std::unique_ptr<Geometry> GeometryFactory::buildGeometry(std::vector<std::unique_ptr<Point>> &&geoms) const {
	if (geoms.empty()) {
		return createGeometryCollection();
	}
...
std::unique_ptr<Geometry> GeometryFactory::buildGeometry(std::vector<std::unique_ptr<LineString>> &&geoms) const {
	if (geoms.empty()) {
		return createGeometryCollection();
	}
...
std::unique_ptr<Geometry> GeometryFactory::buildGeometry(std::vector<std::unique_ptr<Polygon>> &&geoms) const {
	if (geoms.empty()) {
		return createGeometryCollection();
	}
...
/*public*/
Geometry *GeometryFactory::buildGeometry(const std::vector<const Geometry *> &fromGeoms) const {
	if (fromGeoms.empty()) {
		return createGeometryCollection().release();
	}
...
/*public*/
std::unique_ptr<Point> GeometryFactory::createPoint(std::size_t coordinateDimension) const {
...
std::unique_ptr<Point> GeometryFactory::createPoint(const CoordinateXY &coordinate) const {
...
/*public*/
Point *GeometryFactory::createPoint(const Coordinate &coordinate) const {
	if (coordinate.isNull()) {
...
/*public*/
Point *GeometryFactory::createPoint(CoordinateSequence *newCoords) const {
	return new Point(newCoords, this);
}

/*public*/
Point *GeometryFactory::createPoint(const CoordinateSequence &fromCoords) const {
	auto newCoords = fromCoords.clone();
	return new Point(newCoords.release(), this);
}

/*public*/
std::unique_ptr<Polygon> GeometryFactory::createPolygon(std::size_t coordinateDimension) const {
...
/*public*/
Polygon *GeometryFactory::createPolygon(LinearRing *shell, std::vector<LinearRing *> *holes) const {
	return new Polygon(shell, holes, this);
}

std::unique_ptr<Polygon> GeometryFactory::createPolygon(std::unique_ptr<LinearRing> &&shell) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<Polygon>(new Polygon(std::move(shell), *this));
}

/*public*/
std::unique_ptr<Polygon> GeometryFactory::createPolygon(std::vector<Coordinate> &&coords) const {
...
std::unique_ptr<Polygon> GeometryFactory::createPolygon(std::unique_ptr<LinearRing> &&shell,
...
/*public*/
Polygon *GeometryFactory::createPolygon(const LinearRing &shell, const std::vector<LinearRing *> &holes) const {
...
/*public*/
std::unique_ptr<LineString> GeometryFactory::createLineString(std::size_t coordinateDimension) const {
	if (coordinateDimension == 3) {
...
/*public*/
std::unique_ptr<LineString> GeometryFactory::createLineString(const LineString &ls) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<LineString>(new LineString(ls));
}

/*public*/
LineString *GeometryFactory::createLineString(CoordinateSequence *newCoords) const {
	return new LineString(newCoords, this);
}

/*public*/
std::unique_ptr<LineString> GeometryFactory::createLineString(CoordinateSequence::Ptr &&newCoords) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<LineString>(new LineString(std::move(newCoords), *this));
}

/*public*/
std::unique_ptr<LineString> GeometryFactory::createLineString(std::vector<Coordinate> &&newCoords) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<LineString>(new LineString(std::move(newCoords), *this));
}

/*public*/
LineString *GeometryFactory::createLineString(const CoordinateSequence &fromCoords) const {
	auto newCoords = fromCoords.clone();
...
/*public*/
std::unique_ptr<LinearRing> GeometryFactory::createLinearRing() const {
	return std::unique_ptr<LinearRing>(new LinearRing(nullptr, this));
}

/*public*/
LinearRing *GeometryFactory::createLinearRing(CoordinateSequence *newCoords) const {
	return new LinearRing(newCoords, this);
}

std::unique_ptr<LinearRing> GeometryFactory::createLinearRing(CoordinateSequence::Ptr &&newCoords) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<LinearRing>(new LinearRing(std::move(newCoords), *this));
}

/*public*/
std::unique_ptr<LinearRing> GeometryFactory::createLinearRing(std::vector<Coordinate> &&newCoords) const {
	// Can't use make_unique with protected constructor
	return std::unique_ptr<LinearRing>(new LinearRing(std::move(newCoords), *this));
}

/*public*/
LinearRing *GeometryFactory::createLinearRing(const CoordinateSequence &fromCoords) const {
	auto newCoords = fromCoords.clone();
...
/*public*/
MultiPoint *GeometryFactory::createMultiPoint(std::vector<Geometry *> *newPoints) const {
	return new MultiPoint(newPoints, this);
}

std::unique_ptr<MultiPoint> GeometryFactory::createMultiPoint(std::vector<Coordinate> &&newPoints) const {
...
std::unique_ptr<MultiPoint> GeometryFactory::createMultiPoint(std::vector<CoordinateXY> &&newPoints) const {
	std::vector<std::unique_ptr<Geometry>> pts(newPoints.size());
...
std::unique_ptr<MultiPoint> GeometryFactory::createMultiPoint(std::vector<std::unique_ptr<Point>> &&newPoints) const {
...
/*public*/
MultiPoint *GeometryFactory::createMultiPoint(const std::vector<const Geometry *> &fromPoints) const {
...
/*public*/
std::unique_ptr<MultiPoint> GeometryFactory::createMultiPoint() const {
	return std::unique_ptr<MultiPoint>(new MultiPoint(nullptr, this));
}

/*public*/
MultiPoint *GeometryFactory::createMultiPoint(const CoordinateSequence &fromCoords) const {
...
/*public*/
MultiPoint *GeometryFactory::createMultiPoint(const std::vector<Coordinate> &fromCoords) const {
...
/*public*/
std::unique_ptr<MultiPolygon> GeometryFactory::createMultiPolygon() const {
	return std::unique_ptr<MultiPolygon>(new MultiPolygon(nullptr, this));
}

/*public*/
MultiPolygon *GeometryFactory::createMultiPolygon(std::vector<Geometry *> *newPolys) const {
...
/*public*/
MultiPolygon *GeometryFactory::createMultiPolygon(const std::vector<const Geometry *> &fromPolys) const {
...
/*public*/
std::unique_ptr<MultiLineString> GeometryFactory::createMultiLineString() const {
	return std::unique_ptr<MultiLineString>(new MultiLineString(nullptr, this));
}

/*public*/
MultiLineString *GeometryFactory::createMultiLineString(std::vector<Geometry *> *newLines) const {
	return new MultiLineString(newLines, this);
}

/*public*/
MultiLineString *GeometryFactory::createMultiLineString(const std::vector<const Geometry *> &fromLines) const {
	...
	for (std::size_t i = 0; i < fromLines.size(); i++) {
		...
		if (!line) {
			throw geos::util::IllegalArgumentException(
			    "createMultiLineString called with a vector containing non-LineStrings");
		}
...
/*private*/
void GeometryFactory::addRef() const {
	++_refCount;
}

/*private*/
void GeometryFactory::dropRef() const {
	if (!--_refCount) {
		if (_autoDestroy) {
			delete this;
		}
...
/*public*/
std::unique_ptr<Geometry> GeometryFactory::createEmpty(int dimension) const {
	switch (dimension) {
	case -1:
		return createGeometryCollection();
	case 0:
...
/*public*/
std::unique_ptr<GeometryCollection> GeometryFactory::createGeometryCollection() const {
	return std::unique_ptr<GeometryCollection>(new GeometryCollection(nullptr, this));
}

/*public*/
std::unique_ptr<Geometry> GeometryFactory::createEmptyGeometry() const {
	return createGeometryCollection();
}

/*public*/
GeometryCollection *GeometryFactory::createGeometryCollection(std::vector<Geometry *> *newGeoms) const {
	return new GeometryCollection(newGeoms, this);
}

/*public*/
GeometryCollection *GeometryFactory::createGeometryCollection(const std::vector<const Geometry *> &fromGeoms) const {
