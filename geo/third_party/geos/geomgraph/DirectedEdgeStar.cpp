...
/*public*/
void DirectedEdgeStar::updateLabelling(const Label &nodeLabel) {
	EdgeEndStar::iterator endIt = end();
	for (EdgeEndStar::iterator it = begin(); it != endIt; ++it) {
		assert(*it);
		DirectedEdge *de = detail::down_cast<DirectedEdge *>(*it);
...
/*private*/
const std::vector<DirectedEdge *> &DirectedEdgeStar::getResultAreaEdges() {
...
void DirectedEdgeStar::linkMinimalDirectedEdges(EdgeRing *er) {
	...
	for (std::vector<DirectedEdge *>::reverse_iterator i = resultAreaEdgeList.rbegin(),
	                                                   iEnd = resultAreaEdgeList.rend();
	     i != iEnd; ++i) {
...
/*public*/
int DirectedEdgeStar::computeDepths(EdgeEndStar::iterator startIt, EdgeEndStar::iterator endIt, int startDepth) {
