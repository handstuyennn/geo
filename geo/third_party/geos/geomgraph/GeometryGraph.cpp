...
#include <geos/geomgraph/index/EdgeSetIntersector.hpp>
#include <geos/geomgraph/index/SimpleMCSweepLineIntersector.hpp>
...
/*public*/
std::unique_ptr<SegmentIntersector> GeometryGraph::computeSelfNodes(LineIntersector &li, bool computeRingSelfNodes,
                                                                    const Envelope *env) {
	...
	if (env && !env->covers(parentGeom->getEnvelopeInternal())) {
		...
		// cerr << "(computeSelfNodes) Self edges reduced from " << se->size() << " to " << self_edges_copy.size() <<
		// std::endl;
		se = &self_edges_copy;
	}

	bool isRings = dynamic_cast<const LinearRing *>(parentGeom) || dynamic_cast<const Polygon *>(parentGeom) ||
...
/*private*/
void GeometryGraph::addSelfIntersectionNodes(uint8_t p_argIndex) {
	for (Edge *e : *edges) {
...
/*private*/
void GeometryGraph::addSelfIntersectionNode(uint8_t p_argIndex, const Coordinate &coord, Location loc) {
 ...
 */
void GeometryGraph::insertBoundaryPoint(uint8_t p_argIndex, const Coordinate &coord) {
...
void GeometryGraph::insertPoint(uint8_t p_argIndex, const Coordinate &coord, geom::Location onLocation) {
	Node *n = nodes->addNode(coord);
	Label &lbl = n->getLabel();
	if (lbl.isNull()) {
...
GeometryGraph::GeometryGraph(uint8_t newArgIndex, const geom::Geometry *newParentGeom)
    : PlanarGraph(), parentGeom(newParentGeom), useBoundaryDeterminationRule(true),
      boundaryNodeRule(algorithm::BoundaryNodeRule::getBoundaryOGCSFS()), argIndex(newArgIndex),
...
GeometryGraph::GeometryGraph(uint8_t newArgIndex, const geom::Geometry *newParentGeom,
    ...
    : PlanarGraph(), parentGeom(newParentGeom), useBoundaryDeterminationRule(true), boundaryNodeRule(bnr),
      argIndex(newArgIndex), hasTooFewPointsVar(false) {
	if (parentGeom != nullptr) {
		add(parentGeom);
	}
}

Location GeometryGraph::determineBoundary(int boundaryCount) {
	return isInBoundary(boundaryCount) ? Location::BOUNDARY : Location::INTERIOR;
}

/* public static */
Location GeometryGraph::determineBoundary(const algorithm::BoundaryNodeRule &boundaryNodeRule, int boundaryCount) {
...
// throw (UnsupportedOperationException *)
{
	...
	else if (const GeometryCollection *x4 = dynamic_cast<const GeometryCollection *>(g)) {
		addCollection(x4);
	}
 ...
 */
void GeometryGraph::addPolygonRing(const LinearRing *lr, Location cwLeft, Location cwRight)
...
void GeometryGraph::computeSplitEdges(std::vector<Edge *> *edgelist) {
	for (std::vector<Edge *>::iterator i = edges->begin(), endIt = edges->end(); i != endIt; ++i) {
		Edge *e = *i;
		e->eiList.addSplitEdges(edgelist);
	}
}

std::unique_ptr<SegmentIntersector> GeometryGraph::computeEdgeIntersections(GeometryGraph *g, LineIntersector *li,
                                                                            bool includeProper, const Envelope *env) {
...
/*public*/
std::vector<Node *> *GeometryGraph::getBoundaryNodes() {
