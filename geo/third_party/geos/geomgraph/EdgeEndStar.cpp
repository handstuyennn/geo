...
#include <cassert>
#include <geos/algorithm/locate/SimplePointInAreaLocator.hpp>
...
/*public*/
EdgeEndStar::EdgeEndStar() : edgeMap() {
...
/*public*/
const Coordinate &EdgeEndStar::getCoordinate() const {
...
/*public*/
void EdgeEndStar::computeLabelling(std::vector<GeometryGraph *> *geomGraph)
...
/*private*/
void EdgeEndStar::computeEdgeEndLabels(const algorithm::BoundaryNodeRule &boundaryNodeRule) {
...
/*public*/
Location EdgeEndStar::getLocation(uint32_t geomIndex, const Coordinate &p, std::vector<GeometryGraph *> *geom) {
	// compute location only on demand
	if (ptInAreaLocation[geomIndex] == Location::NONE) {
		ptInAreaLocation[geomIndex] =
		    algorithm::locate::SimplePointInAreaLocator::locate(p, (*geom)[geomIndex]->getGeometry());
	}
	return ptInAreaLocation[geomIndex];
}

/*private*/
bool EdgeEndStar::checkAreaLabelsConsistent(uint32_t geomIndex) {
...
/*public*/
void EdgeEndStar::propagateSideLabels(uint32_t geomIndex)
// throw(TopologyException *)
{
	// Since edges are stored in CCW order around the node,
	...
	for (it = beginIt; it != endIt; ++it) {
		...
		if (label.isArea(geomIndex)) {
			Location leftLoc = label.getLocation(geomIndex, Position::LEFT);

			Location rightLoc = label.getLocation(geomIndex, Position::RIGHT);
