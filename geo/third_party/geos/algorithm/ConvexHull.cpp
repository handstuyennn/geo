...
private:
	...
	static int polarCompare(const Coordinate *o, const Coordinate *p, const Coordinate *q) {
...
/* private */
std::unique_ptr<CoordinateSequence> ConvexHull::toCoordinateSequence(Coordinate::ConstVect &cv) {
	const CoordinateSequenceFactory *csf = geomFactory->getCoordinateSequenceFactory();
...
std::unique_ptr<Geometry> ConvexHull::getConvexHull() {
...
/* private */
void ConvexHull::reduce(Coordinate::ConstVect &pts) {
...
/* private */
void ConvexHull::padArray3(geom::Coordinate::ConstVect &pts) {
...
/* private */
void ConvexHull::preSort(Coordinate::ConstVect &pts) {
...
/* private */
void ConvexHull::computeOctPts(const Coordinate::ConstVect &p_inputPts, Coordinate::ConstVect &pts) {
...
/* private */
bool ConvexHull::computeOctRing(const Coordinate::ConstVect &p_inputPts, Coordinate::ConstVect &dest) {
...
/*private*/
void ConvexHull::grahamScan(const Coordinate::ConstVect &c, Coordinate::ConstVect &ps) {
	ps.push_back(c[0]);
	ps.push_back(c[1]);
	ps.push_back(c[2]);
	...
	for (std::size_t i = 3, n = c.size(); i < n; ++i) {
		...
		ps.pop_back();
		while (!ps.empty() && Orientation::index(*(ps.back()), *p, *(c[i])) > 0) {
			p = ps.back();
			ps.pop_back();
		}
		ps.push_back(p);
...
/* private */
std::unique_ptr<Geometry> ConvexHull::lineOrPolygon(const Coordinate::ConstVect &input) {
...
/*private*/
void ConvexHull::cleanRing(const Coordinate::ConstVect &original, Coordinate::ConstVect &cleaned) {
...
/*private*/
bool ConvexHull::isBetween(const Coordinate &c1, const Coordinate &c2, const Coordinate &c3) {
