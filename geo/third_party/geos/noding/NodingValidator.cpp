...
/*private*/
void NodingValidator::checkCollapses() const {
	for (SegmentString::NonConstVect::const_iterator it = segStrings.begin(), itEnd = segStrings.end(); it != itEnd;
	     ++it) {
...
/* private */
void NodingValidator::checkCollapses(const SegmentString &ss) const {
...
/* private */
void NodingValidator::checkCollapse(const Coordinate &p0, const Coordinate &p1, const Coordinate &p2) const {
	if (p0.equals2D(p2))
		throw util::TopologyException("found non-noded collapse at Coordinate, Coordinate, Coordinate");
}
...
void NodingValidator::checkInteriorIntersections() {
	for (SegmentString::NonConstVect::const_iterator it = segStrings.begin(), itEnd = segStrings.end(); it != itEnd;
		...
		for (SegmentString::NonConstVect::const_iterator j = segStrings.begin(), jEnd = segStrings.end(); j != jEnd;
		     ++j) {
...
/* private */
void NodingValidator::checkInteriorIntersections(const SegmentString &ss0, const SegmentString &ss1) {
...
/* private */
void NodingValidator::checkInteriorIntersections(const SegmentString &e0, std::size_t segIndex0,
...
/* private */
void NodingValidator::checkEndPtVertexIntersections() const {
	for (SegmentString::NonConstVect::const_iterator it = segStrings.begin(), itEnd = segStrings.end(); it != itEnd;
	     ++it) {
		const SegmentString *ss = *it;
		const CoordinateSequence &pts = *(ss->getCoordinates());
...
/* private */
void NodingValidator::checkEndPtVertexIntersections(const Coordinate &testPt,
                                                    const SegmentString::NonConstVect &p_segStrings) const {
	for (SegmentString::NonConstVect::const_iterator it = p_segStrings.begin(), itEnd = p_segStrings.end(); it != itEnd;
	     ++it) {
...
/* private */
bool NodingValidator::hasInteriorIntersection(const LineIntersector &aLi, const Coordinate &p0,
