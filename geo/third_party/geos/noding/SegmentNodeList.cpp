...
#include <geos/geom/CoordinateArraySequence.hpp> // FIXME: should we really be using this ?
...
#include <geos/noding/SegmentNodeList.hpp>
#include <geos/noding/SegmentString.hpp> // for use
...
void SegmentNodeList::add(const Coordinate &intPt, std::size_t segmentIndex) {
...
void SegmentNodeList::prepare() const {
	if (!ready) {
		std::sort(nodeMap.begin(), nodeMap.end(),
		          [](const SegmentNode &s1, const SegmentNode &s2) { return s1.compareTo(s2) < 0; });
...
/* private */
void SegmentNodeList::addCollapsedNodes() {
...
/* private */
void SegmentNodeList::findCollapsesFromExistingVertices(std::vector<size_t> &collapsedVertexIndexes) const {
	if (edge.size() < 2) {
...
/* private */
void SegmentNodeList::findCollapsesFromInsertedNodes(std::vector<size_t> &collapsedVertexIndexes) const {
...
/* private */
bool SegmentNodeList::findCollapseIndex(const SegmentNode &ei0, const SegmentNode &ei1, size_t &collapsedVertexIndex) {
...
/*private*/
void SegmentNodeList::checkSplitEdgesCorrectness(const std::vector<SegmentString *> &splitEdges) const {
	if (splitEdges.empty())
		return;
	...
	if (!(pt0 == edgePts->getAt(0))) {
		throw util::GEOSException("bad split edge start point at Coordinate");
	}
	...
	if (!(ptn == edgePts->getAt(edgePts->getSize() - 1))) {
		throw util::GEOSException("bad split edge end point at Coordinate");
	}
}

/*private*/
std::unique_ptr<SegmentString> SegmentNodeList::createSplitEdge(const SegmentNode *ei0, const SegmentNode *ei1) const {
	auto pts = createSplitEdgePts(ei0, ei1);
	return detail::make_unique<NodedSegmentString>(pts.release(), edge.getData());
}

/*private*/
std::unique_ptr<CoordinateSequence> SegmentNodeList::createSplitEdgePts(const SegmentNode *ei0,
                                                                        const SegmentNode *ei1) const {
...
/*public*/
std::vector<Coordinate> SegmentNodeList::getSplitCoordinates() {
...
/*private*/
void SegmentNodeList::addEdgeCoordinates(const SegmentNode *ei0, const SegmentNode *ei1,
                                         std::vector<Coordinate> &coordList) const {
	auto pts = createSplitEdgePts(ei0, ei1);
