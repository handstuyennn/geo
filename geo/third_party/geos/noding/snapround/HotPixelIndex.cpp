...
#include <geos/noding/snapround/HotPixelIndex.hpp>
#include <memory>
#include <random>
...
HotPixelIndex::HotPixelIndex(const PrecisionModel *p_pm)
    : pm(p_pm), scaleFactor(p_pm->getScale()), index(new KdTree()) {
}

/*public*/
HotPixel *HotPixelIndex::add(const Coordinate &p) {
...
/*public*/
void HotPixelIndex::add(const CoordinateSequence *pts) {
	/*
...
/*public*/
void HotPixelIndex::add(const std::vector<geom::Coordinate> &pts) {
	std::vector<std::size_t> idxs;
	for (std::size_t i = 0, sz = pts.size(); i < sz; i++)
		idxs.push_back(i);
...
/*public*/
void HotPixelIndex::addNodes(const CoordinateSequence *pts) {
	for (std::size_t i = 0, sz = pts->size(); i < sz; i++) {
...
/*public*/
void HotPixelIndex::addNodes(const std::vector<geom::Coordinate> &pts) {
	for (auto pt : pts) {
		HotPixel *hp = add(pt);
		hp->setToNode();
	}
...
/*private*/
HotPixel *HotPixelIndex::find(const geom::Coordinate &pixelPt) {
...
/*private*/
Coordinate HotPixelIndex::round(const Coordinate &pt) {
	Coordinate p2 = pt;
	pm->makePrecise(p2);
	return p2;
}
...
/*public*/
void HotPixelIndex::query(const Coordinate &p0, const Coordinate &p1, index::kdtree::KdNodeVisitor &visitor) {
