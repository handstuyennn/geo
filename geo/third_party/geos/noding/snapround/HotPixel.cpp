...
HotPixel::HotPixel(const Coordinate &newPt, double newScaleFactor)
    : originalPt(newPt), scaleFactor(newScaleFactor), hpIsNode(false), hpx(newPt.x), hpy(newPt.y) {
	if (scaleFactor <= 0.0) {
...
/*public*/
const geom::Coordinate &HotPixel::getCoordinate() const {
	return originalPt;
}

/* public */
bool HotPixel::intersects(const Coordinate &p) const {
	double x = scale(p.x);
	double y = scale(p.y);
	if (x >= hpx + TOLERANCE)
		return false;
...
/*public*/
bool HotPixel::intersects(const Coordinate &p0, const Coordinate &p1) const {
	if (scaleFactor == 1.0) {
...
/*private*/
bool HotPixel::intersectsScaled(double p0x, double p0y, double p1x, double p1y) const {
...
/*private*/
bool HotPixel::intersectsPixelClosure(const Coordinate &p0, const Coordinate &p1) const {
	LineIntersector li;
