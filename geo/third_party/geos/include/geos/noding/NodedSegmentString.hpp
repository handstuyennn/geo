...
#include <geos/noding/NodableSegmentString.hpp> // for inheritance
#include <geos/noding/NodedSegmentString.hpp>
#include <geos/noding/Octant.hpp>
...
public:
	...
	NodedSegmentString(geom::CoordinateSequence *newPts, const void *newContext)
	    : NodableSegmentString(newContext), nodeList(this), pts(newPts) {
	}

	NodedSegmentString(SegmentString *ss)
	    : NodableSegmentString(ss->getData()), nodeList(this), pts(ss->getCoordinates()->clone()) {
	}
	 ...
	 */
	void addIntersection(algorithm::LineIntersector *li, std::size_t segmentIndex, std::size_t geomIndex,
	                     std::size_t intIndex) {
