...
public:
	 ...
	 */
	Envelope() : minx(DoubleNotANumber), maxx(DoubleNotANumber), miny(DoubleNotANumber), maxy(DoubleNotANumber) {};
	 ...
	 */
	static bool intersects(const CoordinateXY &p1, const CoordinateXY &p2, const CoordinateXY &q1,
	...
	double distanceSquared(const Envelope &env) const {
		double dx =
		    std::max(0.0, std::max(maxx, env.maxx) - std::min(minx, env.minx) - (maxx - minx) - (env.maxx - env.minx));
		double dy =
		    std::max(0.0, std::max(maxy, env.maxy) - std::min(miny, env.miny) - (maxy - miny) - (env.maxy - env.miny));
	...
	GEOS_DLL friend bool operator==(const Envelope &a, const Envelope &b) {
		return a.equals(&b);
	}
