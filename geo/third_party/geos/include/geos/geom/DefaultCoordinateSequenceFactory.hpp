...
public:
	...
	std::unique_ptr<CoordinateSequence> create(std::vector<Coordinate> *coords,
	                                           std::size_t dims = 0) const final override {
	...
	std::unique_ptr<CoordinateSequence> create(std::vector<CoordinateXY> &&coords,
	                                           std::size_t dims = 0) const final override {
		return detail::make_unique<CoordinateArraySequence>(std::move(coords), dims);
	}
