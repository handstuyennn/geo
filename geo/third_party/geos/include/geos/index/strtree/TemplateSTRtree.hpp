...
#include <geos/index/strtree/Interval.hpp>
#include <geos/index/strtree/TemplateSTRNode.hpp>
...
public:
	...
	public:
		...
		Iterator(typename NodeList::const_iterator &&iter, typename NodeList::const_iterator &&end)
	...
	explicit TemplateSTRtreeImpl(size_t p_nodeCapacity = 10)
	    : root(nullptr), nodeCapacity(p_nodeCapacity), numItems(0) {
	}
	...
	TemplateSTRtreeImpl(const TemplateSTRtreeImpl &other)
	    : root(other.root), nodeCapacity(other.nodeCapacity), numItems(other.numItems) {
		nodes = other.nodes;
	}
	...
	void query(const BoundsType &queryEnv, std::vector<ItemType> &results) {
		query(queryEnv, [&results](const ItemType &x) { results.push_back(x); });
	}
...
protected:
	...
	size_t treeSize(size_t numLeafNodes) {
		...
		while (nodesWithoutParents > 1) {
			...
			for (size_t j = 0; j < numSlices; j++) {
				...
				parentNodesAdded += static_cast<size_t>(
				    std::ceil(static_cast<double>(nodesInSlice) / static_cast<double>(nodeCapacity)));
			}
	...
	// continue.
	template <typename Visitor,
	          typename std::enable_if<std::is_void<decltype(std::declval<Visitor>()(std::declval<ItemType>()))>::value,
	...
	template <typename Visitor,
	          typename std::enable_if<std::is_void<decltype(std::declval<Visitor>()(std::declval<BoundsType>(),
	                                                                                std::declval<ItemType>()))>::value,
	                                  std::nullptr_t>::type = nullptr>
	bool visitLeaf(Visitor &&visitor, const Node &node) {
...
template <typename ItemType>
class TemplateSTRtree<ItemType *, EnvelopeTraits> : public TemplateSTRtreeImpl<ItemType *, EnvelopeTraits>,
                                                    public SpatialIndex {
public:
	...
	void query(const geom::Envelope *queryEnv, std::vector<void *> &results) override {
		query(*queryEnv,
		      [&results](const ItemType *x) { results.push_back(const_cast<void *>(static_cast<const void *>(x))); });
	}

	void query(const geom::Envelope *queryEnv, ItemVisitor &visitor) override {
		query(*queryEnv,
		      [&visitor](const ItemType *x) { visitor.visitItem(const_cast<void *>(static_cast<const void *>(x))); });
	}
