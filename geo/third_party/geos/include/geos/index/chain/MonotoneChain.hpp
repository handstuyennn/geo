...
#include <geos/geom/CoordinateSequence.hpp> // for inline
#include <geos/geom/Envelope.hpp>           // for inline
#include <geos/geom/LineSegment.hpp>        // for inline
...
private:
	void computeSelect(const geom::Envelope &searchEnv, std::size_t start0, std::size_t end0,
	                   MonotoneChainSelectAction &mcs) const;

	void computeOverlaps(std::size_t start0, std::size_t end0, const MonotoneChain &mc, std::size_t start1,
	                     std::size_t end1, double overlapTolerance, MonotoneChainOverlapAction &mco) const;

	bool overlaps(std::size_t start0, std::size_t end0, const MonotoneChain &mc, std::size_t start1, std::size_t end1,
	              double overlapTolerance) const {
		if (overlapTolerance > 0.0) {
			return overlaps(pts->getAt(start0), pts->getAt(end0), mc.pts->getAt(start1), mc.pts->getAt(end1),
			                overlapTolerance);
		}
		return geom::Envelope::intersects(pts->getAt(start0), pts->getAt(end0), mc.pts->getAt(start1),
		                                  mc.pts->getAt(end1));
	}

	static bool overlaps(const geom::Coordinate &p1, const geom::Coordinate &p2, const geom::Coordinate &q1,
	                     const geom::Coordinate &q2, double overlapTolerance);
