...
private:
	 ...
	 */
	static std::vector<std::unique_ptr<MaximalEdgeRing>> buildMaximalRings(const std::vector<OverlayEdge *> &edges);
	 ...
	 */
	void placeFreeHoles(const std::vector<OverlayEdgeRing *> &shellList,
	                    const std::vector<OverlayEdgeRing *> &freeHoleList) const;

public:
	PolygonBuilder(std::vector<OverlayEdge *> &resultAreaEdges, const geom::GeometryFactory *geomFact)
	    : geometryFactory(geomFact), isEnforcePolygonal(true) {
		buildRings(resultAreaEdges);
	}

	PolygonBuilder(std::vector<OverlayEdge *> &resultAreaEdges, const geom::GeometryFactory *geomFact,
	               bool p_isEnforcePolygonal)
	    : geometryFactory(geomFact), isEnforcePolygonal(p_isEnforcePolygonal) {
		buildRings(resultAreaEdges);
	}
