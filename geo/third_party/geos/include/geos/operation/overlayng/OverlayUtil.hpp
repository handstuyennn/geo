...
private:
	 ...
	 */
	static bool resultEnvelope(int opCode, const InputGeometry *inputGeom, const PrecisionModel *pm,
...
public:
	 ...
	 */
	static bool clippingEnvelope(int opCode, const InputGeometry *inputGeom, const PrecisionModel *pm,
	                             Envelope &rsltEnvelope);
	 ...
	 */
	static std::unique_ptr<Geometry> createResultGeometry(std::vector<std::unique_ptr<Polygon>> &resultPolyList,
	 ...
	 */
	static bool isResultAreaConsistent(const Geometry *geom0, const Geometry *geom1, int opCode,
	...
	template <typename T>
	static void moveGeometry(std::vector<std::unique_ptr<T>> &inGeoms,
