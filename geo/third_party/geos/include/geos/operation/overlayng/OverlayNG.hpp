...
public:
	 ...
	 */
	OverlayNG(const geom::Geometry *geom0, const geom::Geometry *geom1, const geom::GeometryFactory *p_geomFact,
	          int p_opCode)
	    : pm(p_geomFact->getPrecisionModel()), inputGeom(geom0, geom1), geomFact(p_geomFact), opCode(p_opCode),
	      noder(nullptr), isStrictMode(STRICT_MODE_DEFAULT), isOptimized(true), isAreaResultOnly(false),
	      isOutputEdges(false), isOutputResultEdges(false), isOutputNodedEdges(false) {
	}
	...
	OverlayNG(const geom::Geometry *geom0, const geom::Geometry *geom1, const geom::PrecisionModel *p_pm, int p_opCode)
	    : pm(p_pm), inputGeom(geom0, geom1), geomFact(geom0->getFactory()), opCode(p_opCode), noder(nullptr),
	      isStrictMode(STRICT_MODE_DEFAULT), isOptimized(true), isAreaResultOnly(false), isOutputEdges(false),
	      isOutputResultEdges(false), isOutputNodedEdges(false) {
	}
	...
	OverlayNG(const geom::Geometry *geom0, const geom::PrecisionModel *p_pm) : OverlayNG(geom0, nullptr, p_pm, UNION) {
	}
