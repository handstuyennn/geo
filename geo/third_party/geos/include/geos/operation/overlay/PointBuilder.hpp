...
#include <geos/geom/GeometryFactory.hpp>        // for inlines
#include <geos/operation/overlay/OverlayOp.hpp> // for OpCode enum
#include <vector>
...
public:
	PointBuilder(OverlayOp *newOp, const geom::GeometryFactory *newGeometryFactory,
	             algorithm::PointLocator *newPtLocator = nullptr)
	    : op(newOp), geometryFactory(newGeometryFactory), resultPointList(new std::vector<geom::Point *>()) {
