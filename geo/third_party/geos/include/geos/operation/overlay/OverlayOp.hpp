...
#include <geos/algorithm/PointLocator.hpp> // for composition
#include <geos/export.hpp>
#include <geos/geom/Dimension.hpp> // for Dimension::DimensionType
#include <geos/geom/Location.hpp>
#include <geos/geomgraph/EdgeList.hpp>               // for composition
#include <geos/geomgraph/PlanarGraph.hpp>            // for inline (GeometryGraph->PlanarGraph)
#include <geos/operation/GeometryGraphOperation.hpp> // for inheritance
#include <vector>
...
public:
	 ...
	 */
	static std::unique_ptr<geom::Geometry> createEmptyResult(OverlayOp::OpCode overlayOpCode, const geom::Geometry *a,
...
private:
	 ...
	 */
	geom::Geometry *computeGeometry(std::vector<geom::Point *> *nResultPointList,
	                                std::vector<geom::LineString *> *nResultLineList,
	                                std::vector<geom::Polygon *> *nResultPolyList, OverlayOp::OpCode opCode);

	/**
	 * For empty result, what is the correct geometry type to apply to
	 * the empty?
	 */
	static geom::Dimension::DimensionType resultDimension(OverlayOp::OpCode overlayOpCode, const geom::Geometry *g0,
