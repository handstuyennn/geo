...
#include <geos/operation/valid/PolygonRingSelfNode.hpp>
...
private:
	 ...
	 */
	const CoordinateXY *scanForHoleCycle(PolygonRingTouch *currentTouch, PolygonRing *root,
...
public:
	...
	PolygonRing(const LinearRing *p_ring, int p_index, PolygonRing *p_shell)
	    : id(p_index), shell(p_shell), ring(p_ring) {};

	/**
	 * Creates a ring for a polygon shell.
	 * @param p_ring
	 */
	PolygonRing(const LinearRing *p_ring) : PolygonRing(p_ring, -1, this) {};
	...
	void addSelfTouch(const CoordinateXY &origin, const CoordinateXY *e00, const CoordinateXY *e01,
