...
private:
	...
	int findInvalidIntersection(const SegmentString *ss0, std::size_t segIndex0, const SegmentString *ss1,
	                            std::size_t segIndex1);

	bool addDoubleTouch(const SegmentString *ss0, const SegmentString *ss1, const CoordinateXY &intPt);

	void addSelfTouch(const SegmentString *ss, const CoordinateXY &intPt, const CoordinateXY *e00,
	                  const CoordinateXY *e01, const CoordinateXY *e10, const CoordinateXY *e11);

	const CoordinateXY &prevCoordinateInRing(const SegmentString *ringSS, std::size_t segIndex) const;

	bool isAdjacentInRing(const SegmentString *ringSS, std::size_t segIndex0, std::size_t segIndex1) const;
...
public:
	...
	PolygonIntersectionAnalyzer(bool p_isInvertedRingValid)
	    : isInvertedRingValid(p_isInvertedRingValid), invalidLocation(CoordinateXY::getNull()),
	      doubleTouchLocation(CoordinateXY::getNull()) {
	}

	void processIntersections(SegmentString *ss0, std::size_t segIndex0, SegmentString *ss1,
