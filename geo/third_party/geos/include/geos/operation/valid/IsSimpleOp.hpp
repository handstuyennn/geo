...
private:
	...
	private:
		...
		bool findIntersection(noding::SegmentString *ss0, std::size_t segIndex0, noding::SegmentString *ss1,
		                      std::size_t segIndex1, const geom::Coordinate &p00, const geom::Coordinate &p01,
		 ...
		 */
		bool isIntersectionEndpoint(const noding::SegmentString *ss, std::size_t ssIndex,
		 ...
		 */
		std::size_t intersectionVertexIndex(const algorithm::LineIntersector &li, std::size_t segmentIndex) const;

	public:
		NonSimpleIntersectionFinder(bool p_isClosedEndpointsInInterior, bool p_isFindAll,
		                            std::vector<geom::Coordinate> &p_intersectionPts)
		    : isClosedEndpointsInInterior(p_isClosedEndpointsInInterior), isFindAll(p_isFindAll),
		...
		void processIntersections(noding::SegmentString *ss0, std::size_t segIndex0, noding::SegmentString *ss1,
...
public:
	IsSimpleOp(const geom::Geometry *geom) : IsSimpleOp(*geom) {};
	 ...
	 */
	IsSimpleOp(const geom::Geometry &geom) : IsSimpleOp(geom, algorithm::BoundaryNodeRule::getBoundaryRuleMod2()) {};
	...
	IsSimpleOp(const geom::Geometry &geom, const algorithm::BoundaryNodeRule &p_boundaryNodeRule)
	    : inputGeom(geom), isClosedEndpointsInInterior(!p_boundaryNodeRule.isInBoundary(2)), isFindAllLocations(false),
	      computed(false) {};
