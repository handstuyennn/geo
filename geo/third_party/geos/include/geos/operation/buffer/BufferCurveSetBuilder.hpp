...
private:
	 ...
	 *                   (if it is CW)
	 */
	void addRingSide(const geom::CoordinateSequence *coord, double offsetDistance, int side, geom::Location cwLeftLoc,
	 ...
	 */
	static bool isRingCurveInverted(const geom::CoordinateSequence *inputPts, double dist,
	 ...
	 */
	static double maxDistance(const geom::CoordinateSequence *pts, const geom::CoordinateSequence *line);
...
public:
	/// Constructor
	BufferCurveSetBuilder(const geom::Geometry &newInputGeom, double newDistance, const geom::PrecisionModel *newPm,
	                      const BufferParameters &newBufParams)
	    : inputGeom(newInputGeom), distance(newDistance), curveBuilder(newPm, newBufParams), curveList(),
	...
	///
	void addCurves(const std::vector<geom::CoordinateSequence *> &lineList, geom::Location leftLoc,
