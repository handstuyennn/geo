...
public:
	...
	OffsetCurveBuilder(const geom::PrecisionModel *newPrecisionModel, const BufferParameters &nBufParams)
	    : distance(0.0), precisionModel(newPrecisionModel), bufParams(nBufParams) {
	}
	 ...
	 *                 be pushed_back
	 */
	void getRingCurve(const geom::CoordinateSequence *inputPts, int side, double distance,
	                  std::vector<geom::CoordinateSequence *> &lineList);
...
private:
	...
	void computeSingleSidedBufferCurve(const geom::CoordinateSequence &inputPts, bool isRightSide,
