...
#include <geos/operation/buffer/BufferOp.hpp>           // for inlines (BufferOp enums)
#include <geos/operation/buffer/OffsetCurveBuilder.hpp> // for inline (OffsetCurveBuilder enums)
#include <vector>
...
public:
	...
	BufferBuilder(const BufferParameters &nBufParams)
	    : bufParams(nBufParams), workingPrecisionModel(nullptr), li(nullptr), intersectionAdder(nullptr),
	      workingNoder(nullptr), geomFact(nullptr), edgeList(), isInvertOrientation(false) {
	}
