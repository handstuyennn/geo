...
#include <geos/geom/CoordinateSequence.hpp> // complete type required
#include <memory>
...
public:
	 ...
	 */
	static std::unique_ptr<geom::CoordinateSequence> simplify(const geom::CoordinateSequence &inputLine,
	                                                          double distanceTol);
...
private:
	 ...
	 * @return
	 */
	bool isShallowSampled(const geom::Coordinate &p0, const geom::Coordinate &p2, std::size_t i0, std::size_t i2,
	                      double distanceTol) const;

	bool isShallow(const geom::Coordinate &p0, const geom::Coordinate &p1, const geom::Coordinate &p2,
	               double distanceTol) const;
