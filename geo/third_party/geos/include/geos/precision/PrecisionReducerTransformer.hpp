...
private:
	...
	void extend(std::vector<geom::Coordinate> &coords, std::size_t minLength);

public:
	PrecisionReducerTransformer(const geom::PrecisionModel &p_targetPM, bool p_isRemoveCollapsed = false)
	    : targetPM(p_targetPM), isRemoveCollapsed(p_isRemoveCollapsed) {};
...
protected:
	std::unique_ptr<geom::CoordinateSequence> transformCoordinates(const geom::CoordinateSequence *coords,
	...
	std::unique_ptr<geom::Geometry> transformMultiPolygon(const geom::MultiPolygon *geom,
	                                                      const geom::Geometry *parent) override;
};
