...
private:
	...
	geom::GeometryFactory::Ptr createFactory(const geom::GeometryFactory &oldGF, const geom::PrecisionModel &newPM);
	 ...
	 */
	std::unique_ptr<geom::Geometry> changePM(const geom::Geometry *geom, const geom::PrecisionModel &newPM);
...
public:
	...
	static std::unique_ptr<geom::Geometry> reducePointwise(const geom::Geometry &g,
	                                                       const geom::PrecisionModel &precModel);

	static std::unique_ptr<geom::Geometry> reduceKeepCollapsed(const geom::Geometry &g,
	                                                           const geom::PrecisionModel &precModel);

	GeometryPrecisionReducer(const geom::PrecisionModel &pm)
	    : newFactory(nullptr), targetPM(pm), removeCollapsed(true), changePrecisionModel(false), useAreaReducer(false),
	      isPointwise(false) {
	}
	...
	GeometryPrecisionReducer(const geom::GeometryFactory &changeFactory)
	    : newFactory(&changeFactory), targetPM(*(changeFactory.getPrecisionModel())), removeCollapsed(true),
	      changePrecisionModel(false), useAreaReducer(false), isPointwise(false) {
	}
