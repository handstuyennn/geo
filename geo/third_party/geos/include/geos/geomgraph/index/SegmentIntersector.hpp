...
private:
	...
	bool isBoundaryPoint(algorithm::LineIntersector *p_li, std::array<std::vector<Node *> *, 2> &tstBdyNodes) {
...
public:
	...
	SegmentIntersector(algorithm::LineIntersector *newLi, bool newIncludeProper, bool newRecordIsolated)
	    : hasIntersectionVar(false), hasProper(false), hasProperInterior(false), li(newLi),
	      includeProper(newIncludeProper), recordIsolated(newRecordIsolated),
	      numIntersections(0), bdyNodes {{nullptr, nullptr}}, numTests(0) {
	}
