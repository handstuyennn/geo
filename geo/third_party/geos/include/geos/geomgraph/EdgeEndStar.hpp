...
#include <geos/geomgraph/EdgeEnd.hpp> // for EdgeEndLT
#include <set>
#include <string>
#include <vector>
...
private:
	virtual geom::Location getLocation(uint32_t geomIndex, const geom::Coordinate &p,
...
inline std::size_t EdgeEndStar::getDegree() {
	return edgeMap.size();
}

inline EdgeEndStar::iterator EdgeEndStar::begin() {
	return edgeMap.begin();
}

inline EdgeEndStar::container &EdgeEndStar::getEdges() {
	return edgeMap;
}

inline EdgeEndStar::reverse_iterator EdgeEndStar::rend() {
	return edgeMap.rend();
}

inline EdgeEndStar::iterator EdgeEndStar::end() {
	return edgeMap.end();
}

inline EdgeEndStar::reverse_iterator EdgeEndStar::rbegin() {
	return edgeMap.rbegin();
}

inline EdgeEndStar::iterator EdgeEndStar::find(EdgeEnd *eSearch) {
