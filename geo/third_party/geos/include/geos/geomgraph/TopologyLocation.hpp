...
public:
	 ...
	 * @see Location
	 */
	TopologyLocation(Location on, Location left, Location right) : locationSize(3) {
		location[Position::ON] = on;
		location[Position::LEFT] = left;
		location[Position::RIGHT] = right;
	}

	TopologyLocation(Location on) : locationSize(1) {
		location.fill(Location::NONE);
		location[Position::ON] = on;
	};

	TopologyLocation(const TopologyLocation &gl) : location(gl.location), locationSize(gl.locationSize) {};
