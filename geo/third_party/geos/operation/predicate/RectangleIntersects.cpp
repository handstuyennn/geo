...
#include <geos/operation/predicate/SegmentIntersectionTester.hpp>
...
#include <geos/geom/util/ShortCircuitedGeometryVisitor.hpp>
#include <memory>
...
public:
	EnvelopeIntersectsVisitor(const geom::Envelope &env) : rectEnv(env), intersectsVar(false) {
	}
...
public:
	ContainsPointVisitor(const geom::Polygon &rect)
	    : rectEnv(*(rect.getEnvelopeInternal())), containsPointVar(false),
...
public:
	LineIntersectsVisitor(const geom::Polygon &rect)
	    : // rectangle(rect),
	      rectEnv(*(rect.getEnvelopeInternal())), rectLine(*(rect.getExteriorRing())), intersectsVar(false)
	// rectSeq(*(rect.getExteriorRing()->getCoordinatesRO()))
	{
	}
...
bool RectangleIntersects::intersects(const geom::Geometry &geom) {
