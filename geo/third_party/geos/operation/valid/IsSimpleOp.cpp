...
/* public static */
bool IsSimpleOp::isSimple(const Geometry &geom) {
	IsSimpleOp op(geom);
	return op.isSimple();
}

/* public */
bool IsSimpleOp::isSimple() {
	compute();
	return isSimpleResult;
}

/* private */
void IsSimpleOp::compute() {
...
/* private */
bool IsSimpleOp::computeSimple(const Geometry &geom) {
	if (geom.isEmpty())
		return true;
...
/* private */
bool IsSimpleOp::isSimpleMultiPoint(const MultiPoint &mp) {
	if (mp.isEmpty())
		return true;
	bool bIsSimple = true;
...
/* private */
bool IsSimpleOp::isSimplePolygonal(const Geometry &geom) {
	bool bIsSimple = true;
...
/* private */
bool IsSimpleOp::isSimpleGeometryCollection(const Geometry &geom) {
	bool bIsSimple = true;
...
/* private static */
std::vector<std::unique_ptr<CoordinateArraySequence>> IsSimpleOp::removeRepeatedPts(const Geometry &geom) {
...
IsSimpleOp::createSegmentStrings(std::vector<std::unique_ptr<CoordinateArraySequence>> &seqs) {
	...
	for (auto &seq : seqs) {
		BasicSegmentString *bss = new BasicSegmentString(seq.get(), nullptr);
...
/* public */
bool IsSimpleOp::NonSimpleIntersectionFinder::hasIntersection() const {
	return intersectionPts.size() > 0;
}

/* public */
void IsSimpleOp::NonSimpleIntersectionFinder::processIntersections(SegmentString *ss0, std::size_t segIndex0,
	...
	bool hasInt = findIntersection(ss0, segIndex0, ss1, segIndex1, p00, p01, p10, p11);
...
/* private */
bool IsSimpleOp::NonSimpleIntersectionFinder::findIntersection(SegmentString *ss0, std::size_t segIndex0,
                                                               SegmentString *ss1, std::size_t segIndex1,
	 ...
	 */
	std::size_t segIndexDiff = segIndex1 > segIndex0 ? segIndex1 - segIndex0 : segIndex0 - segIndex1;
	bool isSameSegString = ss0 == ss1;
...
/* private */
bool IsSimpleOp::NonSimpleIntersectionFinder::isIntersectionEndpoint(const SegmentString *ss, std::size_t ssIndex,
                                                                     const LineIntersector &lineInter,
                                                                     std::size_t liSegmentIndex) const {
...
/* private */
std::size_t IsSimpleOp::NonSimpleIntersectionFinder::intersectionVertexIndex(const LineIntersector &lineInter,
...
/* public */
bool IsSimpleOp::NonSimpleIntersectionFinder::isDone() const {
	if (isFindAll)
		return false;
	return intersectionPts.size() > 0;
}
