...
#include <geos/index/SpatialIndex.hpp>
...
IndexedNestedPolygonTester::IndexedNestedPolygonTester(const MultiPolygon *p_multiPoly)
    : multiPoly(p_multiPoly), nestedPt(Coordinate::getNull()) {
	loadIndex();
}

/* private */
void IndexedNestedPolygonTester::loadIndex() {
...
/* public */
bool IndexedNestedPolygonTester::isNested() {
	for (std::size_t i = 0; i < multiPoly->getNumGeometries(); i++) {
		const Polygon *poly = multiPoly->getGeometryN(i);
...
/* private */
bool IndexedNestedPolygonTester::findNestedPoint(const LinearRing *shell, const Polygon *possibleOuterPoly,
                                                 IndexedPointInAreaLocator &locator, CoordinateXY &coordNested) {
	/**
...
/* private */
IndexedPointInAreaLocator &IndexedNestedPolygonTester::getLocator(const Polygon *poly) {
	...
	if (search == locators.end()) {
		...
		locators.emplace(std::piecewise_construct, std::forward_as_tuple(poly), std::forward_as_tuple(*poly));
 ...
 */
/* private static */
bool IndexedNestedPolygonTester::findIncidentSegmentNestedPoint(const LinearRing *shell, const Polygon *poly,
