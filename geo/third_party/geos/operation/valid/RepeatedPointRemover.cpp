...
public:
	RepeatedPointFilter() : m_prev(nullptr), sqTolerance(0.0) {
	}

	RepeatedPointFilter(double tolerance) : m_prev(nullptr), sqTolerance(tolerance * tolerance) {
	}

	void filter_ro(const Coordinate *curr) override final {

		// skip duplicate point or too-close poinnt
		if (m_prev != nullptr && (curr->equals2D(*m_prev) || curr->distanceSquared(*m_prev) <= sqTolerance)) {
			return;
		}
...
/* public static */
std::unique_ptr<CoordinateArraySequence> RepeatedPointRemover::removeRepeatedPoints(const CoordinateSequence *seq,
                                                                                    double tolerance) {
...
public:
	RepeatedInvalidPointFilter() : m_prev(nullptr), sqTolerance(0.0) {
	}

	RepeatedInvalidPointFilter(double tolerance) : m_prev(nullptr), sqTolerance(tolerance * tolerance) {
	}
	...
	void filter_ro(const Coordinate *curr) override final {
		...
		if (m_prev != nullptr &&
		    (invalid || curr->equals2D(*m_prev) || curr->distanceSquared(*m_prev) <= sqTolerance)) {
			return;
		}
...
public:
	RepeatedPointCoordinateOperation() : tolerance(0.0) {};

	RepeatedPointCoordinateOperation(double p_tolerance) : tolerance(p_tolerance) {};

	std::unique_ptr<CoordinateSequence> edit(const CoordinateSequence *coordinates, const Geometry *geom) override {
		if (!coordinates)
			return nullptr;
...
/* public static */
std::unique_ptr<Geometry> RepeatedPointRemover::removeRepeatedPoints(const Geometry *inputGeom, double tolerance) {
