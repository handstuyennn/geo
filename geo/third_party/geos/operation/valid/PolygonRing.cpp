...
/* public static */
bool PolygonRing::addTouch(PolygonRing *ring0, PolygonRing *ring1, const CoordinateXY &pt) {
...
/* private */
bool PolygonRing::isOnlyTouch(const PolygonRing *polyRing, const CoordinateXY &pt) const {
...
/* public */
void PolygonRing::addSelfTouch(const CoordinateXY &origin, const CoordinateXY *e00, const CoordinateXY *e01,
                               const CoordinateXY *e10, const CoordinateXY *e11) {
	selfNodes.emplace_back(origin, e00, e01, e10, e11);
}

/* public static */
const CoordinateXY *PolygonRing::findInteriorSelfNode(std::vector<PolygonRing *> polyRings) {
...
/* public static */
const CoordinateXY *PolygonRing::findHoleCycleLocation(std::vector<PolygonRing *> polyRings) {
	for (PolygonRing *polyRing : polyRings) {
		if (!polyRing->isInTouchSet()) {
			const CoordinateXY *holeCycleLoc = polyRing->findHoleCycleLocation();
			if (holeCycleLoc != nullptr)
				return holeCycleLoc;
		}
	}
	return nullptr;
...
/* private static */
void PolygonRing::init(PolygonRing *root, std::stack<PolygonRingTouch *> &touchStack) {
...
/* private */
void PolygonRing::addTouch(PolygonRing *polyRing, const CoordinateXY &pt) {
	...
	if (nTouches == 0) {
		...
		touches.emplace(std::piecewise_construct, std::forward_as_tuple(polyRing->id),
		                std::forward_as_tuple(polyRing, pt));
	};
}

/* private */
std::vector<PolygonRingTouch *> PolygonRing::getTouches() const {
...
/* private */
const CoordinateXY *PolygonRing::findHoleCycleLocation() {
...
/* private */
const CoordinateXY *PolygonRing::scanForHoleCycle(PolygonRingTouch *currentTouch, PolygonRing *root,
                                                  std::stack<PolygonRingTouch *> &touchStack) {
...
/* public */
const CoordinateXY *PolygonRing::findInteriorSelfNode() {
	if (selfNodes.empty())
		return nullptr;
