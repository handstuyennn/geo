...
#include <geos/algorithm/PolygonNodeTopology.hpp>
...
#include <geos/noding/BasicSegmentString.hpp>
#include <geos/noding/MCIndexNoder.hpp>
#include <geos/noding/SegmentString.hpp>
#include <geos/operation/valid/PolygonIntersectionAnalyzer.hpp>
...
PolygonTopologyAnalyzer::PolygonTopologyAnalyzer(const Geometry *geom, bool p_isInvertedRingValid)
    : isInvertedRingValid(p_isInvertedRingValid), segInt(p_isInvertedRingValid),
...
/* private static */
std::vector<PolygonRing *> PolygonTopologyAnalyzer::getPolygonRings(const std::vector<SegmentString *> &segStrings) {
...
/* private static */
std::vector<SegmentString *> PolygonTopologyAnalyzer::createSegmentStrings(const Geometry *geom,
                                                                           bool bIsInvertedRingValid) {
...
/* private */
PolygonRing *PolygonTopologyAnalyzer::createPolygonRing(const LinearRing *p_ring) {
	polyRingStore.emplace_back(p_ring);
	return &(polyRingStore.back());
}

/* private */
PolygonRing *PolygonTopologyAnalyzer::createPolygonRing(const LinearRing *p_ring, int p_index, PolygonRing *p_shell) {
	polyRingStore.emplace_back(p_ring, p_index, p_shell);
	return &(polyRingStore.back());
}

/* public static */
CoordinateXY PolygonTopologyAnalyzer::findSelfIntersection(const LinearRing *ring) {
...
/* public static */
bool PolygonTopologyAnalyzer::isRingNested(const LinearRing *test, const LinearRing *target) {
...
/* private static */
const CoordinateXY &PolygonTopologyAnalyzer::findNonEqualVertex(const LinearRing *ring, const CoordinateXY &p) {
	std::size_t i = 1;
...
/* private static */
const CoordinateXY &PolygonTopologyAnalyzer::findRingVertexPrev(const CoordinateSequence *ringPts, std::size_t index,
...
/* private static */
std::size_t PolygonTopologyAnalyzer::ringIndexPrev(const CoordinateSequence *ringPts, std::size_t index) {
	if (index == 0) {
...
/* private static */
std::size_t PolygonTopologyAnalyzer::ringIndexNext(const CoordinateSequence *ringPts, std::size_t index) {
...
/* private static */
const CoordinateXY &PolygonTopologyAnalyzer::findRingVertexNext(const CoordinateSequence *ringPts, std::size_t index,
                                                                const CoordinateXY &node) {
...
/* private static */
bool PolygonTopologyAnalyzer::isIncidentSegmentInRing(const CoordinateXY *p0, const CoordinateXY *p1,
...
/* private static */
std::size_t PolygonTopologyAnalyzer::intersectingSegIndex(const CoordinateSequence *ringPts, const CoordinateXY *pt) {
...
/* public */
bool PolygonTopologyAnalyzer::isInteriorDisconnected() {
	/**
...
/* private static */
SegmentString *PolygonTopologyAnalyzer::createSegString(const LinearRing *ring, const PolygonRing *polyRing) {
...
/* public */
void PolygonTopologyAnalyzer::checkInteriorDisconnectedBySelfTouch() {
	if (!polyRings.empty()) {
...
/* public */
void PolygonTopologyAnalyzer::checkInteriorDisconnectedByHoleCycle() {
	/**
