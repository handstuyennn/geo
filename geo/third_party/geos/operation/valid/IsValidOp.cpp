...
#include <geos/operation/valid/IndexedNestedHoleTester.hpp>
#include <geos/operation/valid/IndexedNestedPolygonTester.hpp>
...
/* public */
const TopologyValidationError *IsValidOp::getValidationError() {
...
/* private */
bool IsValidOp::isValidGeometry(const Geometry *g) {
...
/* private */
bool IsValidOp::isValid(const Point *g) {
...
/* private */
void IsValidOp::checkCoordinatesValid(const CoordinateSequence *coords) {
	for (std::size_t i = 0; i < coords->size(); i++) {
		if (!isValid(coords->getAt(i))) {
			logInvalid(TopologyValidationError::eInvalidCoordinate, coords->getAt(i));
			return;
		}
	}
}
...
/* private */
void IsValidOp::checkCoordinatesValid(const Polygon *poly) {
...
/* private */
bool IsValidOp::isValid(const MultiPoint *g) {
	for (std::size_t i = 0; i < g->getNumGeometries(); i++) {
		const Point *p = g->getGeometryN(i);
		if (p->isEmpty())
			continue;
		if (!isValid(p->getCoordinate())) {
			logInvalid(TopologyValidationError::eInvalidCoordinate, *(p->getCoordinate()));
			return false;
			;
		}
	}
...
/* private */
bool IsValidOp::isValid(const LineString *g) {
	checkCoordinatesValid(g->getCoordinatesRO());
	if (hasInvalidError())
		return false;
...
/* private */
bool IsValidOp::isValid(const LinearRing *g) {
	checkCoordinatesValid(g->getCoordinatesRO());
	if (hasInvalidError())
		return false;
...
/* private */
bool IsValidOp::isValid(const Polygon *g) {
...
/* private */
bool IsValidOp::isValid(const MultiPolygon *g) {
	for (std::size_t i = 0; i < g->getNumGeometries(); i++) {
...
/* private */
bool IsValidOp::isValid(const GeometryCollection *gc) {
...
/* public static */
bool IsValidOp::isValid(const CoordinateXY *coord) {
...
void IsValidOp::logInvalid(int code, const CoordinateXY &pt) {
	validErr = detail::make_unique<TopologyValidationError>(code, pt);
}

/* private */
void IsValidOp::checkAreaIntersections(PolygonTopologyAnalyzer &areaAnalyzer) {
...
/* private */
void IsValidOp::checkRingClosed(const LinearRing *ring) {
	if (ring->isEmpty())
		return;
	if (!ring->isClosed()) {
		Coordinate pt = ring->getNumPoints() >= 1 ? ring->getCoordinateN(0) : Coordinate();
...
/* private */
void IsValidOp::checkRingsClosed(const Polygon *poly) {
...
/* private */
void IsValidOp::checkRingsPointSize(const Polygon *poly) {
...
/* private */
void IsValidOp::checkRingPointSize(const LinearRing *ring) {
	if (ring->isEmpty())
		return;
...
/* private */
void IsValidOp::checkRingSimple(const LinearRing *ring) {
	CoordinateXY intPt = PolygonTopologyAnalyzer::findSelfIntersection(ring);
	if (!intPt.isNull()) {
		logInvalid(TopologyValidationError::eRingSelfIntersection, intPt);
	}
}

/* private */
void IsValidOp::checkTooFewPoints(const LineString *line, std::size_t minSize) {
	if (!isNonRepeatedSizeAtLeast(line, minSize)) {
		Coordinate pt = line->getNumPoints() >= 1 ? line->getCoordinateN(0) : Coordinate();
		logInvalid(TopologyValidationError::eTooFewPoints, pt);
	}
}

/* private */
bool IsValidOp::isNonRepeatedSizeAtLeast(const LineString *line, std::size_t minSize) {
	std::size_t numPts = 0;
...
/* private */
void IsValidOp::checkHolesInShell(const Polygon *poly) {
	...
	for (std::size_t i = 0; i < poly->getNumInteriorRing(); i++) {
		...
		if (invalidPt != nullptr) {
			logInvalid(TopologyValidationError::eHoleOutsideShell, *invalidPt);
			return;
		}
	}
}
...
/* private */
const CoordinateXY *IsValidOp::findHoleOutsideShellPoint(const LinearRing *hole, const LinearRing *shell) {
...
/* private */
void IsValidOp::checkHolesNotNested(const Polygon *poly) {
	// skip test if no holes are present
	if (poly->getNumInteriorRing() <= 0)
		return;
	...
	if (nestedTester.isNested()) {
		logInvalid(TopologyValidationError::eNestedHoles, nestedTester.getNestedPoint());
	}
}

/* private */
void IsValidOp::checkShellsNotNested(const MultiPolygon *mp) {
	...
	if (nestedTester.isNested()) {
		logInvalid(TopologyValidationError::eNestedShells, nestedTester.getNestedPoint());
	}
}

/* private */
void IsValidOp::checkInteriorConnected(PolygonTopologyAnalyzer &analyzer) {
	if (analyzer.isInteriorDisconnected())
		logInvalid(TopologyValidationError::eDisconnectedInterior, analyzer.getDisconnectionLocation());
}
