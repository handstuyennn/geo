...
#include <geos/operation/overlayng/InputGeometry.hpp>
...
#include <geos/operation/overlayng/RobustClipEnvelopeComputer.hpp>
...
/*public static*/
bool OverlayUtil::isFloating(const PrecisionModel *pm) {
	if (pm == nullptr)
		return true;
...
/*private static*/
double OverlayUtil::safeExpandDistance(const Envelope *env, const PrecisionModel *pm) {
...
/*private static*/
bool OverlayUtil::safeEnv(const Envelope *env, const PrecisionModel *pm, Envelope &rsltEnvelope) {
...
/*private static*/
bool OverlayUtil::resultEnvelope(int opCode, const InputGeometry *inputGeom, const PrecisionModel *pm,
...
/*public static*/
bool OverlayUtil::clippingEnvelope(int opCode, const InputGeometry *inputGeom, const PrecisionModel *pm,
                                   Envelope &rsltEnvelope) {
	...
	Envelope clipEnv =
	    RobustClipEnvelopeComputer::getEnvelope(inputGeom->getGeometry(0), inputGeom->getGeometry(1), &rsltEnvelope);

	return safeEnv(&clipEnv, pm, rsltEnvelope);
}

/*public static*/
bool OverlayUtil::isEmptyResult(int opCode, const Geometry *a, const Geometry *b, const PrecisionModel *pm) {
	switch (opCode) {
	case OverlayNG::INTERSECTION: {
...
/*private*/
bool OverlayUtil::isEmpty(const Geometry *geom) {
	return geom == nullptr || geom->isEmpty();
}

/*public static*/
bool OverlayUtil::isEnvDisjoint(const Geometry *a, const Geometry *b, const PrecisionModel *pm) {
	if (isEmpty(a) || isEmpty(b)) {
		return true;
	}
	if (isFloating(pm)) {
...
/*private static*/
bool OverlayUtil::isDisjoint(const Envelope *envA, const Envelope *envB, const PrecisionModel *pm) {
...
/*public static*/
std::unique_ptr<Geometry> OverlayUtil::createEmptyResult(int dim, const GeometryFactory *geomFact) {
...
/*public static*/
int OverlayUtil::resultDimension(int opCode, int dim0, int dim1) {
...
/* public static */
bool OverlayUtil::isResultAreaConsistent(const Geometry *geom0, const Geometry *geom1, int opCode,
	...
	case OverlayNG::INTERSECTION:
		isConsistent =
		    isLess(areaResult, areaA, AREA_HEURISTIC_TOLERANCE) && isLess(areaResult, areaB, AREA_HEURISTIC_TOLERANCE);
		break;
	case OverlayNG::DIFFERENCE:
...
/*public static*/
std::unique_ptr<Geometry> OverlayUtil::createResultGeometry(std::vector<std::unique_ptr<Polygon>> &resultPolyList,
...
/*public static*/
std::unique_ptr<Geometry> OverlayUtil::toLines(OverlayGraph *graph, bool isOutputEdges,
...
/*public static*/
bool OverlayUtil::round(const Point *pt, const PrecisionModel *pm, Coordinate &rsltCoord) {
	if (pt->isEmpty())
		return false;
