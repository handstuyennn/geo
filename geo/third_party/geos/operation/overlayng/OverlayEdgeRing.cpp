...
#include <geos/operation/overlayng/OverlayEdgeRing.hpp>
...
OverlayEdgeRing::OverlayEdgeRing(OverlayEdge *start, const GeometryFactory *geometryFactory)
    : startEdge(start), ring(nullptr), m_isHole(false), locator(nullptr), shell(nullptr) {
...
/*public*/
std::unique_ptr<LinearRing> OverlayEdgeRing::getRing() {
	return std::move(ring);
}

const LinearRing *OverlayEdgeRing::getRingPtr() const {
	return ring.get();
}
 ...
 */
/*public*/
bool OverlayEdgeRing::isHole() const {
	return m_isHole;
}
 ...
 */
/*public*/
void OverlayEdgeRing::setShell(OverlayEdgeRing *p_shell) {
	shell = p_shell;
	if (shell != nullptr)
		shell->addHole(this);
}
 ...
 */
/*public*/
bool OverlayEdgeRing::hasShell() const {
 ...
 */
/*public*/
const OverlayEdgeRing *OverlayEdgeRing::getShell() const {
	if (isHole())
		return shell;
	return this;
}
...
/*public*/
void OverlayEdgeRing::addHole(OverlayEdgeRing *p_ring) {
	holes.push_back(p_ring);
}

void OverlayEdgeRing::closeRing(CoordinateArraySequence &pts) {
	if (pts.size() > 0) {
...
/*private*/
void OverlayEdgeRing::computeRingPts(OverlayEdge *start, CoordinateArraySequence &pts) {
...
/*private*/
void OverlayEdgeRing::computeRing(std::unique_ptr<CoordinateArraySequence> &&p_ringPts,
 ...
 */
/*private*/
const CoordinateArraySequence &OverlayEdgeRing::getCoordinates() {
 ...
 */
/*public*/
OverlayEdgeRing *OverlayEdgeRing::findEdgeRingContaining(const std::vector<OverlayEdgeRing *> &erList) {
...
/*private*/
PointOnGeometryLocator *OverlayEdgeRing::getLocator() {
	if (locator == nullptr) {
...
/*public*/
bool OverlayEdgeRing::isInRing(const Coordinate &pt) {
	/**
...
/*public*/
const Coordinate &OverlayEdgeRing::getCoordinate() {
 ...
 */
/*public*/
std::unique_ptr<Polygon> OverlayEdgeRing::toPolygon(const GeometryFactory *factory) {
...
/*public*/
OverlayEdge *OverlayEdgeRing::getEdge() {
	return startEdge;
}
