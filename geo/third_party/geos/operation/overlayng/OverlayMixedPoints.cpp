...
#include <geos/operation/overlayng/OverlayGraph.hpp>
#include <geos/operation/overlayng/OverlayLabel.hpp>
#include <geos/operation/overlayng/OverlayMixedPoints.hpp>
#include <geos/operation/overlayng/OverlayNG.hpp>
#include <geos/operation/overlayng/OverlayUtil.hpp>
...
public:
	CoordinateExtractingFilter(CoordinateArraySequence &p_pts, const PrecisionModel &p_pm) : pts(p_pts), pm(p_pm) {
	}
...
/*public*/
OverlayMixedPoints::OverlayMixedPoints(int p_opCode, const Geometry *geom0, const Geometry *geom1,
                                       const PrecisionModel *p_pm)
    : opCode(p_opCode), pm(p_pm), geometryFactory(geom0->getFactory()),
...
/*public static*/
std::unique_ptr<Geometry> OverlayMixedPoints::overlay(int opCode, const Geometry *geom0, const Geometry *geom1,
                                                      const PrecisionModel *pm) {
...
/*public*/
std::unique_ptr<Geometry> OverlayMixedPoints::getResult() {
...
/*private*/
std::unique_ptr<PointOnGeometryLocator> OverlayMixedPoints::createLocator(const Geometry *p_geomNonPoint) {
...
/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::prepareNonPoint(const Geometry *geomInput) {
...
/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::computeIntersection(const CoordinateArraySequence *coords) const {
	std::vector<std::unique_ptr<Point>> points = findPoints(true, coords);
	return createPointResult(points);
}

/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::computeUnion(const CoordinateArraySequence *coords) {
...
/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::computeDifference(const CoordinateArraySequence *coords) {
	if (isPointRHS) {
...
/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::copyNonPoint() const {
...
/*private*/
std::unique_ptr<Geometry> OverlayMixedPoints::createPointResult(std::vector<std::unique_ptr<Point>> &points) const {
	if (points.size() == 0) {
...
/*private*/
std::vector<std::unique_ptr<Point>> OverlayMixedPoints::findPoints(bool isCovered,
                                                                   const CoordinateArraySequence *coords) const {
...
/*private*/
std::vector<std::unique_ptr<Point>> OverlayMixedPoints::createPoints(std::set<Coordinate> &coords) const {
...
/*private*/
bool OverlayMixedPoints::hasLocation(bool isCovered, const Coordinate &coord) const {
...
/*private*/
std::unique_ptr<CoordinateArraySequence> OverlayMixedPoints::extractCoordinates(const Geometry *points,
                                                                                const PrecisionModel *p_pm) const {
...
/*private*/
std::vector<std::unique_ptr<Polygon>> OverlayMixedPoints::extractPolygons(const Geometry *geom) const {
...
/*private*/
std::vector<std::unique_ptr<LineString>> OverlayMixedPoints::extractLines(const Geometry *geom) const {
