...
#include <geos/operation/overlayng/EdgeNodingBuilder.hpp>
...
/*private*/
Noder *EdgeNodingBuilder::getNoder() {
...
/*private*/
std::unique_ptr<Noder> EdgeNodingBuilder::createFixedPrecisionNoder(const PrecisionModel *p_pm) {
	std::unique_ptr<Noder> srNoder(new SnapRoundingNoder(p_pm));
	return srNoder;
}

/*private*/
std::unique_ptr<Noder> EdgeNodingBuilder::createFloatingPrecisionNoder(bool doValidation) {
...
/*public*/
void EdgeNodingBuilder::setClipEnvelope(const Envelope *p_clipEnv) {
	clipEnv = p_clipEnv;
...
/*public*/
std::vector<Edge *> EdgeNodingBuilder::build(const Geometry *geom0, const Geometry *geom1) {
	add(geom0, 0);
	add(geom1, 1);
...
/*private*/
std::vector<Edge *> EdgeNodingBuilder::node(std::vector<SegmentString *> *segStrings) {
...
/*private*/
std::vector<Edge *> EdgeNodingBuilder::createEdges(std::vector<SegmentString *> *segStrings) {
...
/*public*/
bool EdgeNodingBuilder::hasEdgesFor(uint8_t geomIndex) const {
	assert(geomIndex < 2);
	return hasEdges[geomIndex];
}

/*private*/
void EdgeNodingBuilder::add(const Geometry *g, uint8_t geomIndex) {
...
/*private*/
void EdgeNodingBuilder::addCollection(const GeometryCollection *gc, uint8_t geomIndex) {
...
/*private*/
void EdgeNodingBuilder::addGeometryCollection(const GeometryCollection *gc, uint8_t geomIndex, int expectedDim) {
	for (std::size_t i = 0; i < gc->getNumGeometries(); i++) {
		const Geometry *g = gc->getGeometryN(i);
...
/*private*/
void EdgeNodingBuilder::addPolygon(const Polygon *poly, uint8_t geomIndex) {
...
/*private*/
void EdgeNodingBuilder::addPolygonRing(const LinearRing *ring, bool isHole, uint8_t geomIndex) {
...
/*private*/
const EdgeSourceInfo *EdgeNodingBuilder::createEdgeSourceInfo(uint8_t index) {
...
/*private*/
const EdgeSourceInfo *EdgeNodingBuilder::createEdgeSourceInfo(uint8_t index, int depthDelta, bool isHole) {
	// Concentrate small memory allocations via std::deque and
	// retain ownership of the EdgeSourceInfo* in the EdgeNodingBuilder
...
/*private*/
void EdgeNodingBuilder::addEdge(std::unique_ptr<CoordinateArraySequence> &cas, const EdgeSourceInfo *info) {
...
/*private*/
void EdgeNodingBuilder::addEdge(std::unique_ptr<std::vector<Coordinate>> pts, const EdgeSourceInfo *info) {
...
/*private*/
bool EdgeNodingBuilder::isClippedCompletely(const Envelope *env) {
	if (clipEnv == nullptr)
		return false;
...
/* private */
std::unique_ptr<geom::CoordinateArraySequence> EdgeNodingBuilder::clip(const LinearRing *ring) {
...
/*private*/
std::unique_ptr<CoordinateArraySequence> EdgeNodingBuilder::removeRepeatedPoints(const LineString *line) {
	const CoordinateSequence *pts = line->getCoordinatesRO();
	return RepeatedPointRemover::removeRepeatedPoints(pts);
}

/*private*/
int EdgeNodingBuilder::computeDepthDelta(const LinearRing *ring, bool isHole) {
	/**
...
/*private*/
void EdgeNodingBuilder::addLine(const LineString *line, uint8_t geomIndex) {
...
/*private*/
void EdgeNodingBuilder::addLine(std::unique_ptr<CoordinateArraySequence> &pts, uint8_t geomIndex) {
	/**
	 * Don't add edges that collapse to a point
	 */
	if (pts->size() < 2) {
...
/*private*/
bool EdgeNodingBuilder::isToBeLimited(const LineString *line) const {
	const CoordinateSequence *pts = line->getCoordinatesRO();
...
/*private*/
std::vector<std::unique_ptr<CoordinateArraySequence>> &EdgeNodingBuilder::limit(const LineString *line) {
	const CoordinateSequence *pts = line->getCoordinatesRO();
