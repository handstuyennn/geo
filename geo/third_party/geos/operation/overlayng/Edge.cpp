...
Edge::Edge(CoordinateSequence *p_pts, const EdgeSourceInfo *info)
    : aDim(OverlayLabel::DIM_UNKNOWN), aDepthDelta(0), aIsHole(false), bDim(OverlayLabel::DIM_UNKNOWN), bDepthDelta(0),
      bIsHole(false), pts(p_pts) {
	copyInfo(info);
}

/*public static*/
bool Edge::isCollapsed(const CoordinateSequence *pts) {
 ...
 */
/*private*/
void Edge::initLabel(OverlayLabel &lbl, uint8_t geomIndex, int dim, int depthDelta, bool p_isHole) {
