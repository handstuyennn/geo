...
/*public*/
std::unique_ptr<CoordinateSequence> OverlayEdge::getCoordinatesOriented() {
	if (direction) {
		return pts->clone();
	}
 ...
 */
/*public*/
void OverlayEdge::addCoordinates(CoordinateArraySequence *coords) const {
...
/*public*/
std::string OverlayEdge::resultSymbol() const {
	if (isInResultArea())
		return std::string(" resA");
	if (isInResultLine())
		return std::string(" resL");
	return std::string("");
}
