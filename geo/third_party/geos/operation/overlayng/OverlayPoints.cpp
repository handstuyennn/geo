...
struct PointExtractingFilter : public GeometryComponentFilter {
	...
	static Coordinate roundCoord(const Point *pt, const PrecisionModel *p_pm) {
...
/*public static*/
std::unique_ptr<Geometry> OverlayPoints::overlay(int opCode, const Geometry *geom0, const Geometry *geom1,
                                                 const PrecisionModel *pm) {
...
/*public*/
std::unique_ptr<Geometry> OverlayPoints::getResult() {
...
/*private*/
void OverlayPoints::computeIntersection(std::map<Coordinate, std::unique_ptr<Point>> &map0,
...
/*private*/
void OverlayPoints::computeDifference(std::map<Coordinate, std::unique_ptr<Point>> &map0,
                                      std::map<Coordinate, std::unique_ptr<Point>> &map1,
                                      std::vector<std::unique_ptr<Point>> &rsltList) {
	// for each entry in map0
	for (auto &ent : map0) {
...
/*private*/
void OverlayPoints::computeUnion(std::map<Coordinate, std::unique_ptr<Point>> &map0,
                                 std::map<Coordinate, std::unique_ptr<Point>> &map1,
                                 std::vector<std::unique_ptr<Point>> &rsltList) {
...
/*private*/
std::map<Coordinate, std::unique_ptr<Point>> OverlayPoints::buildPointMap(const Geometry *geom) {
