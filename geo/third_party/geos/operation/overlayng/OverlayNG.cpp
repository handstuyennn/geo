...
#include <geos/operation/overlayng/ElevationModel.hpp>
#include <geos/operation/overlayng/InputGeometry.hpp>
#include <geos/operation/overlayng/IntersectionPointBuilder.hpp>
#include <geos/operation/overlayng/LineBuilder.hpp>
#include <geos/operation/overlayng/OverlayLabeller.hpp>
#include <geos/operation/overlayng/OverlayMixedPoints.hpp>
#include <geos/operation/overlayng/OverlayNG.hpp>
#include <geos/operation/overlayng/OverlayPoints.hpp>
#include <geos/operation/overlayng/OverlayUtil.hpp>
#include <geos/operation/overlayng/PolygonBuilder.hpp>
...
/*public static*/
std::unique_ptr<Geometry> OverlayNG::overlay(const Geometry *geom0, const Geometry *geom1, int opCode,
...
/*public*/
std::unique_ptr<Geometry> OverlayNG::getResult() {
...
/*private*/
std::unique_ptr<Geometry> OverlayNG::computeEdgeOverlay() {
	/**
	...
	std::vector<Edge *> edges = nodingBuilder.build(inputGeom.getGeometry(0), inputGeom.getGeometry(1));
	...
	if (OverlayUtil::isFloating(pm)) {
		bool isAreaConsistent = OverlayUtil::isResultAreaConsistent(inputGeom.getGeometry(0), inputGeom.getGeometry(1),
		                                                            opCode, result.get());
		if (!isAreaConsistent)
...
/*private*/
std::unique_ptr<Geometry> OverlayNG::extractResult(int p_opCode, OverlayGraph *graph) {
	...
	if (!isAreaResultOnly) {
		...
		bool allowResultLines =
		    !hasResultAreaComponents || isAllowMixedIntResult || opCode == SYMDIFFERENCE || opCode == UNION;
	...
	if (resultPolyList.empty() && resultLineList.empty() && resultPointList.empty()) {
		return createEmptyResult();
	}

	std::unique_ptr<Geometry> resultGeom =
	    OverlayUtil::createResultGeometry(resultPolyList, resultLineList, resultPointList, geomFact);
	return resultGeom;
}

/*private*/
void OverlayNG::labelGraph(OverlayGraph *graph) {
...
/*private*/
std::unique_ptr<Geometry> OverlayNG::createEmptyResult() {
...
/*public static*/
bool OverlayNG::isResultOfOp(int overlayOpCode, Location loc0, Location loc1) {
...
/*public static*/
std::unique_ptr<Geometry> OverlayNG::geomunion(const Geometry *geom, const PrecisionModel *pm) {
	OverlayNG ov(geom, pm);
	return ov.getResult();
}

/*public static*/
std::unique_ptr<Geometry> OverlayNG::geomunion(const Geometry *geom, const PrecisionModel *pm, noding::Noder *noder) {
	OverlayNG ov(geom, pm);
	ov.setNoder(noder);
