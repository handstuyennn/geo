...
/*public static*/
std::unique_ptr<Geometry> OverlayNGRobust::Union(const Geometry *g0, const Geometry *g1) {
	return Overlay(g0, g1, OverlayNG::UNION);
}

/*public static*/
std::unique_ptr<Geometry> OverlayNGRobust::Union(const Geometry *a) {
...
/*public static*/
std::unique_ptr<Geometry> OverlayNGRobust::Overlay(const Geometry *geom0, const Geometry *geom1, int opCode) {
...
/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::overlaySnapTries(const Geometry *geom0, const Geometry *geom1, int opCode) {
	std::unique_ptr<Geometry> result;
...
/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::overlaySR(const Geometry *geom0, const Geometry *geom1, int opCode) {
	std::unique_ptr<Geometry> result;
	try {
...
/*public static*/
double OverlayNGRobust::snapTolerance(const Geometry *geom0, const Geometry *geom1) {
...
/*private static*/
double OverlayNGRobust::snapTolerance(const Geometry *geom) {
	double magnitude = ordinateMagnitude(geom);
	return magnitude / SNAP_TOL_FACTOR;
}

/*private static*/
double OverlayNGRobust::ordinateMagnitude(const Geometry *geom) {
	if (geom == nullptr || geom->isEmpty())
		return 0;
	const Envelope *env = geom->getEnvelopeInternal();
	double magMax = std::max(std::abs(env->getMaxX()), std::abs(env->getMaxY()));
	double magMin = std::max(std::abs(env->getMinX()), std::abs(env->getMinY()));
	return std::max(magMax, magMin);
}

/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::overlaySnapping(const Geometry *geom0, const Geometry *geom1, int opCode,
                                                           double snapTol) {
	try {
...
/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::overlaySnapBoth(const Geometry *geom0, const Geometry *geom1, int opCode,
                                                           double snapTol) {
	try {
...
/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::snapSelf(const Geometry *geom, double snapTol) {
...
/*private static*/
std::unique_ptr<Geometry> OverlayNGRobust::overlaySnapTol(const Geometry *geom0, const Geometry *geom1, int opCode,
                                                          double snapTol) {
	noding::snap::SnappingNoder snapNoder(snapTol);
