...
/*public*/
std::vector<std::unique_ptr<LineString>> LineBuilder::getLines() {
	markResultLines();
	addResultLines();
...
/*private*/
void LineBuilder::markResultLines() {
...
/*private*/
bool LineBuilder::isResultLine(const OverlayLabel *lbl) const {
	/**
...
/*private*/
Location LineBuilder::effectiveLocation(const OverlayLabel *lbl, uint8_t geomIndex) {
...
/*private*/
void LineBuilder::addResultLinesMerged() {
	addResultLinesForNodes();
	addResultLinesRings();
}

/*private*/
void LineBuilder::addResultLines() {
	std::vector<OverlayEdge *> &edges = graph->getEdges();
...
std::unique_ptr<LineString> LineBuilder::toLine(OverlayEdge *edge) const {
...
/*private*/
void LineBuilder::addResultLinesForNodes() {
	std::vector<OverlayEdge *> &edges = graph->getEdges();
	for (OverlayEdge *edge : edges) {
		if (!edge->isInResultLine())
			continue;
...
/*private*/
void LineBuilder::addResultLinesRings() {
...
/*private*/
std::unique_ptr<LineString> LineBuilder::buildLine(OverlayEdge *node) {
...
/*private*/
OverlayEdge *LineBuilder::nextLineEdgeUnvisited(OverlayEdge *node) {
	OverlayEdge *e = node;
	do {
		e = e->oNextOE();
		if (e->isVisited()) {
...
/*private*/
int LineBuilder::degreeOfLines(OverlayEdge *node) {
	int degree = 0;
	OverlayEdge *e = node;
	do {
		if (e->isInResultLine()) {
