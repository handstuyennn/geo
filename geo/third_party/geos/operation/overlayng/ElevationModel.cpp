...
/* public static */
std::unique_ptr<ElevationModel> ElevationModel::create(const geom::Geometry &geom1, const geom::Geometry &geom2) {
	Envelope extent;
	if (!geom1.isEmpty()) {
...
/* public static */
std::unique_ptr<ElevationModel> ElevationModel::create(const geom::Geometry &geom1) {
	Envelope extent;
	if (!geom1.isEmpty()) {
		extent.expandToInclude(geom1.getEnvelopeInternal());
	}
...
ElevationModel::ElevationModel(const Envelope &nExtent, int nNumCellX, int nNumCellY)
    : extent(nExtent), numCellX(nNumCellX), numCellY(nNumCellY) {
...
/* public */
void ElevationModel::add(const Geometry &geom) {
...
#if GEOS_DEBUG
			std::cerr << "Coordinate " << i << " of added geom is: " << c << std::endl;
#endif
...
/* protected */
void ElevationModel::add(double x, double y, double z) {
	if (std::isnan(z))
		return;
	hasZValue = true;
...
void ElevationModel::init() {
	...
	for (ElevationCell &cell : cells) {
		if (!cell.isNull()) {
			cell.compute();
#if GEOS_DEBUG
			std::cerr << "init: cell" << offset << " getZ: " << cell.getZ() << std::endl;
#endif
			numCells++;
			sumZ += cell.getZ();
		}
...
#if GEOS_DEBUG
	std::cerr << "init: numCells: " << numCells << " averageZ: " << averageZ << std::endl;
#endif
}

/* public */
double ElevationModel::getZ(double x, double y) {
	if (!isInitialized) {
		init();
	}
...
/* public */
void ElevationModel::populateZ(Geometry &geom) {
	...
	std::cerr << "ElevationModel::poplateZ calling apply_rw(CoordinateSequenceFilter&) against" <<
	    ...
	    typeid(geom).name() << std::endl;
#endif
	geom.apply_rw(&filter);
}

/* private */
ElevationModel::ElevationCell &ElevationModel::getCell(double x, double y) //, bool isCreateIfMissing
{
	int ix = 0;
	if (numCellX > 1) {
...
#if GEOS_DEBUG
	std::cerr << "Cell of coordinate " << x << "," << y << " is " << ix << "," << iy << " offset " << cellOffset
	          << std::endl;
#endif
