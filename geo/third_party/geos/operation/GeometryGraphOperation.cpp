...
#include <geos/operation/GeometryGraphOperation.hpp>
...
GeometryGraphOperation::GeometryGraphOperation(const Geometry *g0, const Geometry *g1) : arg(2) {
	...
	arg[0] = new GeometryGraph(0, g0, algorithm::BoundaryNodeRule::getBoundaryOGCSFS());
	arg[1] = new GeometryGraph(1, g1, algorithm::BoundaryNodeRule::getBoundaryOGCSFS());
}
...
GeometryGraphOperation::GeometryGraphOperation(const Geometry *g0) : arg(1) {
	const PrecisionModel *pm0 = g0->getPrecisionModel();
	assert(pm0);
...
const Geometry *GeometryGraphOperation::getArgGeometry(unsigned int i) const {
	assert(i < arg.size());
