...
#include <cmath>
#include <geos/algorithm/Distance.hpp>           // for use
#include <geos/algorithm/Orientation.hpp>        // for use
#include <geos/geom/CoordinateArraySequence.hpp> // for constructing the return
...
BufferInputLineSimplifier::BufferInputLineSimplifier(const geom::CoordinateSequence &input)
    : inputLine(input), angleOrientation(Orientation::COUNTERCLOCKWISE) {
}

/*public static*/
std::unique_ptr<geom::CoordinateSequence> BufferInputLineSimplifier::simplify(const geom::CoordinateSequence &inputLine,
                                                                              double distanceTol) {
...
/* public */
std::unique_ptr<geom::CoordinateSequence> BufferInputLineSimplifier::simplify(double nDistanceTol) {
...
/* private */
bool BufferInputLineSimplifier::deleteShallowConcavities() {
	/*
...
/* private */
bool BufferInputLineSimplifier::isDeletable(std::size_t i0, std::size_t i1, std::size_t i2,
...
/* private */
bool BufferInputLineSimplifier::isShallowSampled(const geom::Coordinate &p0, const geom::Coordinate &p2, std::size_t i0,
                                                 std::size_t i2, double p_distanceTol) const {
...
/* private */
bool BufferInputLineSimplifier::isShallow(const geom::Coordinate &p0, const geom::Coordinate &p1,
                                          const geom::Coordinate &p2, double p_distanceTol) const {
	double dist = Distance::pointToSegment(p1, p0, p2);
	return dist < p_distanceTol;
}

/* private */
bool BufferInputLineSimplifier::isConcave(const geom::Coordinate &p0, const geom::Coordinate &p1,
...
/* private */
size_t BufferInputLineSimplifier::findNextNonDeletedIndex(std::size_t index) const {
...
/* private */
std::unique_ptr<geom::CoordinateSequence> BufferInputLineSimplifier::collapseLine() const {
