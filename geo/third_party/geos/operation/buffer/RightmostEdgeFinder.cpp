...
#include <geos/operation/buffer/RightmostEdgeFinder.hpp>
...
RightmostEdgeFinder::RightmostEdgeFinder()
    : minIndex(-1), // FIXME: don't use -1 as a sentinel, or we won't be
      ...
      minCoord(Coordinate::getNull()), minDe(nullptr), orientedDe(nullptr) {
}

/*public*/
void RightmostEdgeFinder::findEdge(std::vector<DirectedEdge *> *dirEdgeList) {
	/*
...
void RightmostEdgeFinder::findRightmostEdgeAtNode() {
	...
	if (!minDe->isForward()) {
		...
		const CoordinateSequence *minEdgeCoords = minEdge->getCoordinates();
...
void RightmostEdgeFinder::checkForRightmostCoordinate(DirectedEdge *de) {
	...
	for (std::size_t i = 0; i < n; i++) {
		...
		if (minCoord.isNull() || coord->getAt(i).x > minCoord.x) {
			minDe = de;
			minIndex = (int)i;
