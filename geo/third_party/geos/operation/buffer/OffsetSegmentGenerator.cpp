...
#include <geos/operation/buffer/BufferInputLineSimplifier.hpp>
#include <geos/operation/buffer/BufferParameters.hpp>
#include <geos/operation/buffer/OffsetSegmentGenerator.hpp>
#include <geos/operation/buffer/OffsetSegmentString.hpp>
...
/*public*/
OffsetSegmentGenerator::OffsetSegmentGenerator(const PrecisionModel *newPrecisionModel,
                                               const BufferParameters &nBufParams, double dist)
    : maxCurveSegmentError(0.0), closingSegLengthFactor(1), segList(), distance(dist),
      precisionModel(newPrecisionModel), bufParams(nBufParams), li(), s0(), s1(), s2(), seg0(), seg1(), offset0(),
      offset1(), side(0), _hasNarrowConcaveAngle(false), endCapIndex(0) {
...
/*private*/
void OffsetSegmentGenerator::init(double newDistance) {
...
/*public*/
void OffsetSegmentGenerator::initSideSegments(const Coordinate &nS1, const Coordinate &nS2, int nSide) {
	s1 = nS1;
	s2 = nS2;
	side = nSide;
...
/*private*/
void OffsetSegmentGenerator::computeOffsetSegment(const LineSegment &seg, int p_side, double p_distance,
                                                  LineSegment &offset) {
...
/*public*/
void OffsetSegmentGenerator::addNextSegment(const Coordinate &p, bool addStartPoint) {
...
/* private */
void OffsetSegmentGenerator::addInsideTurn(int orientation, bool addStartPoint) {
	...
	if (offset0.p1.distance(offset1.p0) < distance * INSIDE_TURN_VERTEX_SNAP_DISTANCE_FACTOR) {
		...
		if (closingSegLengthFactor > 0) {
			Coordinate mid0((closingSegLengthFactor * offset0.p1.x + s1.x) / (closingSegLengthFactor + 1),
			                (closingSegLengthFactor * offset0.p1.y + s1.y) / (closingSegLengthFactor + 1));
			segList.addPt(mid0);

			Coordinate mid1((closingSegLengthFactor * offset1.p0.x + s1.x) / (closingSegLengthFactor + 1),
...
/* private */
void OffsetSegmentGenerator::addMitreJoin(const geom::Coordinate &cornerPt, const geom::LineSegment &p_offset0,
                                          const geom::LineSegment &p_offset1, double p_distance) {
...
/* private */
void OffsetSegmentGenerator::addLimitedMitreJoin(const geom::LineSegment &p_offset0, const geom::LineSegment &p_offset1,
                                                 double p_distance, double p_mitreLimitDistance) {
...
/* private static */
LineSegment OffsetSegmentGenerator::extend(const LineSegment &seg, double dist) {
...
/*public*/
void OffsetSegmentGenerator::addLineEndCap(const Coordinate &p0, const Coordinate &p1) {
	...
	case BufferParameters::CAP_ROUND:
		...
		addDirectedFillet(p1, angle + MATH_PI / 2.0, angle - MATH_PI / 2.0, Orientation::CLOCKWISE, distance);
	...
	case BufferParameters::CAP_SQUARE:
		...
		Coordinate squareCapLOffset(offsetL.p1.x + squareCapSideOffset.x, offsetL.p1.y + squareCapSideOffset.y);
		Coordinate squareCapROffset(offsetR.p1.x + squareCapSideOffset.x, offsetR.p1.y + squareCapSideOffset.y);
...
/* private */
void OffsetSegmentGenerator::addCollinear(bool addStartPoint) {
	/*
	...
	if (numInt >= 2) {
		...
		if (bufParams.getJoinStyle() == BufferParameters::JOIN_BEVEL ||
			...
			addDirectedFillet(s1, offset0.p1, offset1.p0, Orientation::CLOCKWISE, distance);
		}
	}
}

/* private */
void OffsetSegmentGenerator::addOutsideTurn(int orientation, bool addStartPoint) {
	/*
...
/*private*/
void OffsetSegmentGenerator::createCircle(const Coordinate &p, double p_distance) {
	// add start point
...
/*private*/
void OffsetSegmentGenerator::addDirectedFillet(const Coordinate &p, const Coordinate &p0, const Coordinate &p1,
...
/*private*/
void OffsetSegmentGenerator::addDirectedFillet(const Coordinate &p, double startAngle, double endAngle, int direction,
                                               double radius) {
...
/*private*/
void OffsetSegmentGenerator::createSquare(const Coordinate &p, double p_distance) {
...
/* private static */
Coordinate OffsetSegmentGenerator::project(const Coordinate &pt, double d, double dir) {
...
/* private */
void OffsetSegmentGenerator::addBevelJoin(const geom::LineSegment &p_offset0, const geom::LineSegment &p_offset1) {
