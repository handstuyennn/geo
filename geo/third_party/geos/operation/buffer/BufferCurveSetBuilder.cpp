...
/* public */
std::vector<SegmentString *> &BufferCurveSetBuilder::getCurves() {
	add(inputGeom);
	return curveList;
}

/*private*/
void BufferCurveSetBuilder::add(const Geometry &g) {
	if (g.isEmpty()) {
		return;
	}
...
/*private*/
void BufferCurveSetBuilder::addCollection(const GeometryCollection *gc) {
...
/*private*/
void BufferCurveSetBuilder::addPoint(const Point *p) {
...
/*public*/
void BufferCurveSetBuilder::addCurves(const std::vector<CoordinateSequence *> &lineList, geom::Location leftLoc,
...
/*private*/
void BufferCurveSetBuilder::addCurve(CoordinateSequence *coord, geom::Location leftLoc, geom::Location rightLoc) {
...
/*private*/
void BufferCurveSetBuilder::addLineString(const LineString *line) {
...
/* private */
void BufferCurveSetBuilder::addRingBothSides(const CoordinateSequence *coord, double p_distance) {
	addRingSide(coord, p_distance, Position::LEFT, Location::EXTERIOR, Location::INTERIOR);
	/* Add the opposite side of the ring
	 */
	addRingSide(coord, p_distance, Position::RIGHT, Location::INTERIOR, Location::EXTERIOR);
}

/* private */
void BufferCurveSetBuilder::addRingSide(const CoordinateSequence *coord, double offsetDistance, int side,
                                        geom::Location cwLeftLoc, geom::Location cwRightLoc) {
...
/* private static*/
bool BufferCurveSetBuilder::isRingCurveInverted(const CoordinateSequence *inputPts, double dist,
	 ...
	 */
	if (inputPts->size() >= MAX_INVERTED_RING_SIZE)
		return false;
	 ...
	 */
	if (curvePts->size() > INVERTED_CURVE_VERTEX_FACTOR * inputPts->size())
		return false;
...
/*private*/
bool BufferCurveSetBuilder::isErodedCompletely(const LinearRing *ring, double bufferDistance) {
...
/*private*/
bool BufferCurveSetBuilder::isTriangleErodedCompletely(const CoordinateSequence *triangleCoord, double bufferDistance) {
 ...
 */
/* private static */
double BufferCurveSetBuilder::maxDistance(const CoordinateSequence *pts, const CoordinateSequence *line) {
...
/*private*/
void BufferCurveSetBuilder::addPolygon(const Polygon *p) {
	...
	auto shellCoord = operation::valid::RepeatedPointRemover::removeRepeatedAndInvalidPoints(shell->getCoordinatesRO());
	...
	addRingSide(shellCoord.get(), offsetDistance, offsetSide, Location::EXTERIOR, Location::INTERIOR);
	...
	for (std::size_t i = 0, n = p->getNumInteriorRing(); i < n; ++i) {
		...
		addRingSide(holeCoord.get(), offsetDistance, Position::opposite(offsetSide), Location::INTERIOR,
		            Location::EXTERIOR);
	}
}

/*private*/
bool BufferCurveSetBuilder::isRingCCW(const CoordinateSequence *coords) const {
