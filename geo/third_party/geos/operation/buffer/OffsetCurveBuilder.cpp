...
#include <geos/operation/buffer/OffsetCurveBuilder.hpp>
...
/*public*/
void OffsetCurveBuilder::getLineCurve(const CoordinateSequence *inputPts, double nDistance,
...
/*private*/
void OffsetCurveBuilder::computeLineBufferCurve(const CoordinateSequence &inputPts, OffsetSegmentGenerator &segGen) {
	double distTol = simplifyTolerance(distance);

	//--------- compute points for left side of line
	// Simplify the appropriate side of the line before generating
	std::unique_ptr<CoordinateSequence> simp1_ = BufferInputLineSimplifier::simplify(inputPts, distTol);
	...
	// Simplify the appropriate side of the line before generating
	std::unique_ptr<CoordinateSequence> simp2_ = BufferInputLineSimplifier::simplify(inputPts, -distTol);
...
/*public*/
bool OffsetCurveBuilder::isLineOffsetEmpty(double p_distance) {
	...
	if (p_distance < 0.0 && !bufParams.isSingleSided())
		return true;
	return false;
}

/*public*/
void OffsetCurveBuilder::getRingCurve(const CoordinateSequence *inputPts, int side, double nDistance,
                                      std::vector<CoordinateSequence *> &lineList) {
	distance = nDistance;
...
void OffsetCurveBuilder::computeRingBufferCurve(const CoordinateSequence &inputPts, int side,
	...
	std::unique_ptr<CoordinateSequence> simp_ = BufferInputLineSimplifier::simplify(inputPts, distTol);
...
/* private */
void OffsetCurveBuilder::computePointCurve(const Coordinate &pt, OffsetSegmentGenerator &segGen) {
...
/*private*/
std::unique_ptr<OffsetSegmentGenerator> OffsetCurveBuilder::getSegGen(double dist) {
	std::unique_ptr<OffsetSegmentGenerator> osg(new OffsetSegmentGenerator(precisionModel, bufParams, dist));
	return osg;
}

/*private*/
void OffsetCurveBuilder::computeSingleSidedBufferCurve(const CoordinateSequence &inputPts, bool isRightSide,
                                                       OffsetSegmentGenerator &segGen) {
...
/* private */
double OffsetCurveBuilder::simplifyTolerance(double bufDistance) {
