 ...
 * overlay operation
 */
std::vector<Point *> *PointBuilder::build(OverlayOp::OpCode opCode) {
 ...
 */
void PointBuilder::extractNonCoveredResultNodes(OverlayOp::OpCode opCode) {
	...
	for (const auto &entry : nodeMap) {
		...
		if (n->getEdges()->getDegree() == 0 || opCode == OverlayOp::opINTERSECTION) {
...
void PointBuilder::filterCoveredNodeToPoint(const Node *n) {
